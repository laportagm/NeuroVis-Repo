...
func _create_figma_enhanced_ui() -> void:
    ...
    related_structures_container.add_theme_constant_override("separation", UIThemeManager.MARGIN_SMALL)
...
func _setup_figma_styling() -> void:
    ...
    if structure_name_label:
        UIThemeManager.apply_modern_label(structure_name_label, UIThemeManager.FONT_SIZES.h1, title_color, "heading")
    ...
    # ===== ENHANCED RICH TEXT =====
    if description_text:
        UIThemeManager.apply_rich_text_styling(description_text, UIThemeManager.FONT_SIZES.body, "description")
...
func _style_difficulty_badge(difficulty: String) -> void:
    ...
    match difficulty.to_lower():
        "beginner":
            badge_style.bg_color = Color(UIThemeManager.ACCENT_GREEN.r, UIThemeManager.ACCENT_GREEN.g, UIThemeManager.ACCENT_GREEN.b, 0.2)
            difficulty_badge.add_theme_color_override("font_color", UIThemeManager.ACCENT_GREEN)
        "intermediate":
            badge_style.bg_color = Color(UIThemeManager.ACCENT_YELLOW.r, UIThemeManager.ACCENT_YELLOW.g, UIThemeManager.ACCENT_YELLOW.b, 0.2)
            difficulty_badge.add_theme_color_override("font_color", UIThemeManager.ACCENT_YELLOW)
        "advanced":
            badge_style.bg_color = Color(UIThemeManager.ACCENT_RED.r, UIThemeManager.ACCENT_RED.g, UIThemeManager.ACCENT_RED.b, 0.2)
...
func _update_enhanced_functions(functions_array: Array) -> void:
    ...
    if functions_array.is_empty():
        ...
        UIThemeManager.apply_modern_label(placeholder, UIThemeManager.FONT_SIZES.caption, UIThemeManager.TEXT_TERTIARY)
    ...
    for i in range(functions_array.size()):
        ...
        UIThemeManager.animate_entrance(item, i * 0.05, UIThemeManager.ANIMATION.normal, "slide_fade")
...
func _create_structure_chip(structure_name: String, index: int) -> Button:
    ...
    chip_style.bg_color = Color(UIThemeManager.ACCENT_BLUE.r, UIThemeManager.ACCENT_BLUE.g, UIThemeManager.ACCENT_BLUE.b, 0.2)
    ...
    UIThemeManager.apply_modern_label(chip, UIThemeManager.FONT_SIZES.caption, UIThemeManager.ACCENT_BLUE)
...
func _animate_figma_entrance() -> void:
    ...
    # Slide in from right
    tween.tween_property(self, "position:x", position.x - 50, 0.4).set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
...
func _on_bookmark_pressed() -> void:
    ...
    # Visual feedback
    UIThemeManager.animate_button_press(bookmark_button, bookmark_active_color if is_bookmarked else bookmark_inactive_color)
...
func clear_data() -> void:
    ...
    if description_text:
        description_text.text = "[color=%s][i]Select a structure to view information[/i][/color]" % UIThemeManager.TEXT_TERTIARY.to_html()
...
func _animate_exit() -> void:
    ...
    tween.tween_property(self, "position:x", position.x + 30, 0.25).set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_IN)
