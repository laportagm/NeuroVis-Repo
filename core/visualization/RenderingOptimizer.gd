...
func optimize_model(model: Node3D, model_name: String) -> bool:
	...
	print("[RenderingOptimizer] Optimized model: " + model_name + " with " + str(mesh_instances.size()) + " meshes")
	return true
			...
			if mesh_instance and mesh_instance.is_inside_tree():
				for i in range(material_group.original_materials.size()):
					if i < mesh_instance.get_surface_override_material_count():
						mesh_instance.set_surface_override_material(i, material_group.original_materials[i])
...
func _perform_frustum_culling() -> void:
	...
	for obj in _scene_objects:
		...
		var global_aabb = AABB(
			obj.global_transform * aabb.position,
			obj.global_transform.basis * aabb.size
		)
...
func _organize_occlusion_objects() -> void:
	...
	for obj in _scene_objects:
		...
		while parent:
			if parent.name.contains("Model") or parent.name.contains("Brain") or parent.name.contains("Structure"):
...
func _batch_scene_materials() -> void:
	...
	for obj in _scene_objects:
		...
		for i in range(obj.get_surface_override_material_count()):
			...
			for group_id in material_groups:
				...
				if _compare_material_signatures(signature, group.signature) >= material_similarity_threshold:
	...
	for group_id in material_groups:
		...
		_batching_groups[group_id] = {
			...
			"mesh_count": affected_meshes.size()
		}
...
func _setup_culling_for_model(model: Node3D, mesh_instances: Array, model_name: String) -> void:
	...
	if occlusion_culling_enabled:
		...
		_occlusion_objects[model_name] = {
			"objects": mesh_instances.duplicate(),
			"center": center,
			"priority": priority
		}
...
func _compare_material_signatures(sig1: Dictionary, sig2: Dictionary) -> float:
	...
	if sig1.has("shader"):
		...
		var param_similarity = 1.0 - abs(sig1.param_count - sig2.param_count) / max(sig1.param_count, 1.0)
...
func _update_performance_stats() -> void:
	...
	# Get memory usage
	performance_stats.memory_usage = Performance.get_monitor(Performance.MEMORY_STATIC) + Performance.get_monitor(Performance.MEMORY_DYNAMIC)
