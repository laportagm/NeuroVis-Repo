...
func _collect_mesh_instances(node: Node, mesh_names: Array, mesh_info: Dictionary, path: String = "") -> void:
	# Update current path
...
func _compare_with_knowledge_base(mesh_names: Array) -> void:
	...
	for mesh_name in mesh_names:
		...
		print(" - " + mesh_name + ": " + match_type + (("") if matching_id.is_empty() else (" with " + matching_id)))
