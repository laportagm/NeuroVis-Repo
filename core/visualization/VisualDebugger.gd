...
static func _static_init() -> void:
	...
	# ORPHANED REF: immediate_mesh.clear_surfaces()
	# ORPHANED REF: immediate_mesh.surface_begin(Mesh.PRIMITIVE_LINES)
	# ORPHANED REF: immediate_mesh.surface_add_vertex(from)
	# ORPHANED REF: immediate_mesh.surface_add_vertex(to)
	# ORPHANED REF: immediate_mesh.surface_end()

	# FIXED: Orphaned code - var material = StandardMaterial3D.new()
	# ORPHANED REF: material.albedo_color = VisualDebugger.ray_color
	# ORPHANED REF: material.emission_enabled = true
	# ORPHANED REF: material.emission = VisualDebugger.ray_color

	var mesh_instance = MeshInstance3D.new()
	# ORPHANED REF: mesh_instance.mesh = immediate_mesh
	# ORPHANED REF: mesh_instance.material_override = material
	...
	# FIXED: Orphaned code - var box_mesh = BoxMesh.new()
	# ORPHANED REF: box_mesh.size = size

	var mesh_instance_2 = MeshInstance3D.new()
	# ORPHANED REF: mesh_instance.mesh = box_mesh
	mesh_instance.position = position
	mesh_instance.material_override = debug_material.duplicate()

	# FIXED: Orphaned code - var current_scene_3 = Engine.get_main_loop().current_scene
	current_scene.add_child(mesh_instance)
	...
	# ORPHANED REF: sphere_mesh.radius = radius
	# ORPHANED REF: sphere_mesh.height = radius * 2.0

	var mesh_instance_3 = MeshInstance3D.new()
	# ORPHANED REF: mesh_instance.mesh = sphere_mesh
	mesh_instance.position = position
	mesh_instance.material_override = debug_material.duplicate()

	# FIXED: Orphaned code - var current_scene_4 = Engine.get_main_loop().current_scene
	current_scene.add_child(mesh_instance)
	...
	# FIXED: Orphaned code - var original_materials = []
	...
	timer.wait_time = duration
	timer.one_shot = true
	timer.timeout.connect(func(): VisualDebugger.remove_highlight(mesh_instance))
	timer.start()
...
static func remove_highlight(mesh_instance: MeshInstance3D) -> void:
	...
	# FIXED: Orphaned code - var to = from + camera.project_ray_normal(click_position) * length

	# ORPHANED REF: draw_ray(from, to, duration)


	# Create label for all nodes in a scene - useful for debugging structure names
	static func label_all_nodes(
parent: Node3D, recursive: bool = true, filter_class: String = ""
	) -> void:
	_static_init()  # Ensure static variables are initialized
...
# Draw a debug ray from origin to destination
	static func draw_ray(
	# ORPHANED REF: from: Vector3, to: Vector3, duration: float = 1.0, parent: Node3D = null
	) -> MeshInstance3D:
	_static_init()  # Ensure static variables are initialized
	if not Engine.is_editor_hint() and not _enabled:
	return null
...
position: Vector3, size: Vector3 = Vector3.ONE, duration: float = 1.0, parent: Node3D = null
	) -> MeshInstance3D:
	_static_init()  # Ensure static variables are initialized
	if not Engine.is_editor_hint() and not _enabled:
	return null
...
position: Vector3, radius: float = 0.5, duration: float = 1.0, parent: Node3D = null
	) -> MeshInstance3D:
	_static_init()  # Ensure static variables are initialized
	if not Engine.is_editor_hint() and not _enabled:
	return null
...
# Highlight a mesh instance
	static func highlight_mesh(
mesh_instance: MeshInstance3D, color: Color = Color(0, 1, 0, 0.5), duration: float = 0.0
	) -> void:
	_static_init()  # Ensure static variables are initialized
	if not Engine.is_editor_hint() and not _enabled:
	return
	...
	# ORPHANED REF: original_materials.append(mesh_instance.get_surface_override_material(i))
	...
	# Store in dictionary
	# ORPHANED REF: mesh_highlights[mesh_instance] = {"original_materials": original_materials, "timer": null}
	...
	for i in range(mesh_instance.get_surface_override_material_count()):
	# ORPHANED REF: if i < highlight_data.original_materials.size():
	# ORPHANED REF: mesh_instance.set_surface_override_material(i, highlight_data.original_materials[i])
...
static func toggle() -> void:
	set_enabled(not _enabled)
												...
												# Visualize a raycast
	static func visualize_raycast(
click_position: Vector2, camera: Camera3D, length: float = 1000.0, duration: float = 1.0
	) -> void:
	_static_init()  # Ensure static variables are initialized
	if not Engine.is_editor_hint() and not _enabled:
	return
...
# Call the original method
	...
	# ORPHANED REF: print("DEBUG: Could not find _handle_selection method to debug")
