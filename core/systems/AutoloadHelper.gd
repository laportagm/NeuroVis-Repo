...
static func get_knowledge_service() -> Node:
	...
	# FIXED: Orphaned code - var tree = Engine.get_main_loop()
	# FIXED: Orphaned code - var theme_mgr = get_theme_manager()
...
static func apply_theme_to_control(control: Control, style_type: String = "default") -> void:
	# ORPHANED REF: if theme_mgr and theme_mgr.has_method("apply_enhanced_panel_style"):
	# ORPHANED REF: theme_mgr.apply_enhanced_panel_style(control, style_type)
else:
