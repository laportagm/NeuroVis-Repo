...
func _sample_metrics() -> void:
    ...
    current_metrics[MetricType.VERTEX_COUNT] = Performance.get_monitor(Performance.RENDER_TOTAL_PRIMITIVES_IN_FRAME)
    
    # Physics and script time
    current_metrics[MetricType.PHYSICS_TIME] = Performance.get_monitor(Performance.TIME_PHYSICS_PROCESS)
...
func _update_overlay() -> void:
    ...
    # Memory
    var mem_color = _get_metric_color(MetricType.MEMORY_USAGE, current_metrics[MetricType.MEMORY_USAGE])
    text += "[color=%s]Memory: %.1f MB[/color]\n" % [mem_color.to_html(), current_metrics[MetricType.MEMORY_USAGE]]
    
    # Draw calls
    var draw_color = _get_metric_color(MetricType.DRAW_CALLS, current_metrics[MetricType.DRAW_CALLS])
    text += "[color=%s]Draw Calls: %d[/color]\n" % [draw_color.to_html(), current_metrics[MetricType.DRAW_CALLS]]
...
func _get_metric_type_from_key(key: String) -> MetricType:
    ...
    match key:
        ...
        "draw_calls": return MetricType.DRAW_CALLS
        _: return MetricType.FPS
