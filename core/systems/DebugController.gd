...
@onready var test_results_container = $VBoxContainer/TabContainer/TestResults/TestResults/ResultsContainer
...
func _on_run_all_tests_pressed() -> void:
	...
	var direct_tests = ["ui_info_panel_test", "knowledge_base_test", "camera_controls_test", "structure_selection_test"]
...
func _run_test_scene(test_name: String) -> void:
	...
	if test_name == "model_switcher_test" and not FileAccess.file_exists("res://tests/model_switcher_test.tscn"):
		_create_model_switcher_test()
	
	# Check if it's a new test without a TSCN file
	var is_direct_script_test = test_name in ["ui_info_panel_test", "knowledge_base_test", "camera_controls_test", "structure_selection_test"]
