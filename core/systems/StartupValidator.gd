...
func _validate_autoloads() -> void:
	...
	for autoload_name in required_autoloads:
		...
		if node:
			...
			critical_error.emit(ValidationCategory.AUTOLOADS, 
				"Missing autoload: " + autoload_name + " - " + required_autoloads[autoload_name])
		
		index += 1
		validation_progress.emit(ValidationCategory.AUTOLOADS, 
			float(index) / required_autoloads.size())
...
func _validate_resources() -> void:
	...
	for i in range(critical_resources.size()):
		...
		if FileAccess.file_exists(resource_path):
			...
			critical_error.emit(ValidationCategory.RESOURCES,
				"Missing resource: " + resource_path)
		
		validation_progress.emit(ValidationCategory.RESOURCES,
			float(i + 1) / critical_resources.size())
...
func _validate_educational_content() -> void:
	...
	if ks:
		...
		critical_error.emit(ValidationCategory.EDUCATIONAL_CONTENT,
			"KnowledgeService not available")
