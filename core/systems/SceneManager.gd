...
enum LoadingState {
    ...
    TRANSITIONING,   # Visual transition between scenes
...
func change_scene(scene_path: String, transition_params: Dictionary = {}) -> void:
    ...
    if _loading_state != LoadingState.IDLE:
        push_warning("[SceneManager] Scene change already in progress, ignoring request for " + scene_path)
        return
...
func _create_fallback_loading_screen() -> void:
    ...
    panel.color = loading_background_color
