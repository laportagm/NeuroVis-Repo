...
# === ENUMS ===
enum ComparisonType {
    SPECIES,        # Compare across species (human vs. other mammals)
    PATHOLOGICAL,   # Compare normal vs. pathological states
    DEVELOPMENTAL,  # Compare across developmental stages
    FUNCTIONAL      # Compare functionally related structures
}

enum AnnotationStyle {
    TEXT_ONLY,      # Simple text annotations
    ARROWS,         # Arrows pointing to key differences
    HIGHLIGHT,      # Color highlighting of differences
    COMPREHENSIVE   # Full educational annotation package
}
...
func start_comparison(structure_ids: Array, comparison_type: ComparisonType = -1, annotation_style: AnnotationStyle = -1) -> bool:
    ...
    if structure_ids.size() > MAX_COMPARISON_STRUCTURES:
        push_warning("[ComparativeAnatomyService] Too many structures, limiting to " + str(MAX_COMPARISON_STRUCTURES))
...
func _validate_setup() -> bool:
    ...
    if _selection_manager == null:
        push_warning("[ComparativeAnatomyService] BrainStructureSelectionManager not found - limited selection integration")
...
func _generate_annotations(comparison_type: ComparisonType, annotation_style: AnnotationStyle) -> void:
...
func _generate_functional_comparison_annotations(annotations: Dictionary) -> void:
    ...
    for function in shared_functions:
        annotations.similarities.append({
            ...
            "educational_note": "This shared function suggests anatomical and evolutionary relationships."
        })
        ...
        for function in unique_functions[id]:
            if not function.is_empty():
                ...
                annotations.differences.append({
                    ...
                    "educational_note": "This specialized function reflects the structure's distinct role."
                })
...
func _generate_educational_report() -> Dictionary:
    ...
    for id in _active_structure_ids:
        if _active_comparison.structures.has(id):
            ...
            report.structures.append({
                ...
                "description": structure.shortDescription if structure.has("shortDescription") else "",
    ...
    for sim in _current_annotations.similarities:
        report.key_similarities.append({
            "title": sim.title,
