...
func start_educational_session(mode: int, options: Dictionary = {}) -> bool:
    ...
    match mode:
        ...
        1:  # Guided
            session_type = "guided"
            if options.has("starting_system") and enable_system_switching and _brain_system_switcher != null:
...
func generate_recommendation() -> Dictionary:
    ...
    if recommendation.is_empty() and enable_comparative_anatomy and _comparative_anatomy != null and _currently_selected_structures.size() > 0:
        ...
        if _currently_selected_structures.size() == 1:
            ...
            if related_structures.size() > 0:
                ...
                recommendation = {
                    ...
                    "description": "Enhance your understanding by comparing these related structures.",
    ...
    if recommendation.is_empty() and enable_system_switching and _brain_system_switcher != null:
        ...
        recommendation = {
            ...
            "description": "Enhance your understanding by switching to a different brain visualization.",
...
func _validate_setup() -> bool:
    ...
    if _selection_manager == null:
        push_warning("[EducationalModuleCoordinator] BrainStructureSelectionManager not found - limited selection integration")
    ...
    if enable_system_switching:
        ...
        if _brain_system_switcher == null:
            push_warning("[EducationalModuleCoordinator] BrainSystemSwitcher not found - system switching disabled")
    ...
    if enable_comparative_anatomy:
        ...
        if _comparative_anatomy == null:
            push_warning("[EducationalModuleCoordinator] ComparativeAnatomyService not found - comparative anatomy disabled")
    ...
    if enable_learning_pathways:
        ...
        if _learning_pathway == null:
            push_warning("[EducationalModuleCoordinator] LearningPathwayManager not found - learning pathways disabled")
...
func _find_pathways_with_structure(structure_id: String) -> Array:
    ...
    if structure_id in ["cerebrum", "cerebellum", "brainstem", 
    ...
    if structure_id in ["hippocampus", "amygdala", "thalamus", 
                        "fornix", "mammillary_bodies", "cingulate_gyrus", "hypothalamus"]:
        matching_pathways.append("limbic_system")
    
    # Check basal ganglia pathway
    if structure_id in ["caudate_nucleus", "putamen", "globus_pallidus", 
                        "substantia_nigra", "subthalamic_nucleus", "striatum"]:
...
func _get_recommended_system_for_structure(structure_id: String) -> int:
    ...
    var internal_structures = ["hippocampus", "amygdala", "thalamus", "hypothalamus",
                               "caudate_nucleus", "putamen", "globus_pallidus", 
                               "substantia_nigra", "subthalamic_nucleus"]
    ...
    var surface_structures = ["frontal_lobe", "parietal_lobe", 
...
func _save_session_analytics(session: Dictionary) -> void:
    ...
    print("[EducationalModuleCoordinator] Session analytics - Duration: %.1f minutes, Structures viewed: %d, Interactions: %d" % [duration_minutes, structures_count, interactions_count])
