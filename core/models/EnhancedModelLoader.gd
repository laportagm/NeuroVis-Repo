...
func _setup_mesh_instance(mesh: MeshInstance3D, model_def: Dictionary) -> void:
	...
	if mesh.scale.x > 50.0:
		print("[EnhancedModelLoader] Fixing abnormal scale on mesh: %s (was %s)" % [mesh.name, mesh.scale])
...
func _finalize_loading() -> void:
	...
	print("[EnhancedModelLoader] Loading complete: %d/%d models loaded successfully" % [successful_count, total_models])
...
func _register_models_with_switcher() -> void:
	...
	var model_switcher = get_node_or_null("/root/ModelSwitcherGlobal")
	if not model_switcher:
		push_warning("[EnhancedModelLoader] ModelSwitcherGlobal not found - models won't be registered")
		return
