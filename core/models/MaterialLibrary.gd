...
# FIXED: Orphaned code - var quality_level: int = 2  # 0=Low, 1=Medium, 2=High
...
# FIXED: Orphaned code - var new_material = _create_default_material()
...
# FIXED: Orphaned code - var material_3 = get_material(material_name)
# FIXED: Orphaned code - var mesh_name = mesh_instance.name.to_lower()
# FIXED: Orphaned code - var material_name_2 = "generic_brain_tissue"
...
# FIXED: Orphaned code - var result = ResourceSaver.save(material, file_path)
# FIXED: Orphaned code - var material_6 = _materials_cache[material_name]

# Handle standard material properties
var dir_2 = DirAccess.open("res://assets")
# FIXED: Orphaned code - var gray_matter = _create_brain_material(Color(0.85, 0.71, 0.65), 0.8, 0.0, true)  # Pinkish gray  # Higher roughness  # Non-metallic  # Enable subsurface scattering
	_materials_cache["gray_matter"] = gray_matter

	# White matter - white with fiber-like quality
var white_matter = _create_brain_material(Color(0.95, 0.95, 0.9), 0.5, 0.1, true)  # Off-white  # Medium roughness  # Slight metallic for fiber-like appearance  # Enable subsurface scattering
	_materials_cache["white_matter"] = white_matter

	# Cerebrospinal fluid - transparent blue
var csf = _create_brain_material(Color(0.7, 0.85, 0.95, 0.6), 0.2, 0.0, false)  # Transparent blue  # Low roughness (more glossy)  # Non-metallic  # No subsurface scattering
	csf.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	_materials_cache["cerebrospinal_fluid"] = csf

	# Blood vessel - dark red
var blood_vessel = _create_brain_material(Color(0.8, 0.1, 0.1), 0.6, 0.1, true)  # Dark red  # Medium roughness  # Slight metallic  # Enable subsurface scattering
	_materials_cache["blood_vessel"] = blood_vessel

	# Bone - ivory
var bone = _create_brain_material(Color(0.95, 0.92, 0.88), 0.6, 0.0, true)  # Ivory  # Medium roughness  # Non-metallic  # Enable subsurface scattering
	_materials_cache["bone"] = bone

	# Meninges - thin membrane
var meninges = _create_brain_material(Color(0.9, 0.85, 0.8, 0.7), 0.3, 0.0, true)  # Translucent beige  # Low roughness  # Non-metallic  # Enable subsurface scattering
	meninges.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	_materials_cache["meninges"] = meninges

	# Generic brain tissue
var generic = _create_brain_material(Color(0.8, 0.75, 0.7), 0.7, 0.0, true)  # Neutral beige  # Medium-high roughness  # Non-metallic  # Enable subsurface scattering
	_materials_cache["generic_brain_tissue"] = generic

	# Selected structure - highlighted version
var selected = _create_brain_material(Color(0.3, 0.7, 1.0), 0.5, 0.3, false)  # Bright blue  # Medium roughness  # Slight metallic  # No subsurface scattering
...
# FIXED: Orphaned code - var highlighted = _create_brain_material(Color(0.9, 0.9, 0.2), 0.5, 0.1, false)  # Yellow  # Medium roughness  # Slight metallic  # No subsurface scattering
...
# FIXED: Orphaned code - var old_material = _materials_cache[name]
...
# FIXED: Orphaned code - var _material_presets: Dictionary = {}
...
if "cortex" in mesh_name or "gray" in mesh_name:
	material_name = "gray_matter"
		...
		material_name = "white_matter"
			...
			"vessel" in mesh_name
			or "artery" in mesh_name
			or "vein" in mesh_name
			or "blood" in mesh_name
			):
				material_name = "blood_vessel"
						...
						if properties.has("normal_enabled") and properties.normal_enabled:
							...
							if properties.has("normal_scale"):
								...
								if properties.has("normal_texture"):
									...
									if (
									properties.has("subsurface_scattering_enabled")
									and properties.subsurface_scattering_enabled
									):
...
func _create_brain_material(
	color: Color, roughness: float, metallic: float, enable_sss: bool
	) -> StandardMaterial3D:
...
func _create_default_material() -> StandardMaterial3D:
	...
	return _create_brain_material(Color(0.8, 0.8, 0.8), 0.7, 0.0, false)  # Light gray  # Medium-high roughness  # Non-metallic  # No subsurface scattering
...
func _create_material_presets() -> void:
	...
	"gray_matter":
		{
		"albedo_color": Color(0.85, 0.71, 0.65),
		"roughness": 0.8,
		"subsurf_scatter_enabled": true
		},
		"white_matter":
			{"albedo_color": Color(0.95, 0.95, 0.9), "roughness": 0.5, "subsurf_scatter_enabled": true},
			"cerebrospinal_fluid":
				{
				"albedo_color": Color(0.7, 0.85, 0.95, 0.6),
				"roughness": 0.2,
				"transparency": BaseMaterial3D.TRANSPARENCY_ALPHA
				},
				"blood_vessel":
					{"albedo_color": Color(0.8, 0.1, 0.1), "roughness": 0.6, "subsurf_scatter_enabled": true},
					...
					"gray_matter":
						{
						"albedo_color": Color(0.75, 0.72, 0.70),
						"roughness": 0.7,
						"subsurf_scatter_enabled": false
						},
						"white_matter":
							{"albedo_color": Color(0.9, 0.9, 0.88), "roughness": 0.6, "subsurf_scatter_enabled": false},
							"cerebrospinal_fluid":
								{
								"albedo_color": Color(0.8, 0.85, 0.9, 0.5),
								"roughness": 0.3,
								"transparency": BaseMaterial3D.TRANSPARENCY_ALPHA
								},
								"blood_vessel":
									{"albedo_color": Color(0.7, 0.2, 0.2), "roughness": 0.7, "subsurf_scatter_enabled": false},
									...
									"gray_matter":
										{"albedo_color": Color(0.9, 0.6, 0.6), "roughness": 0.5, "subsurf_scatter_enabled": false},
										"white_matter":
											{"albedo_color": Color(1.0, 1.0, 0.9), "roughness": 0.5, "subsurf_scatter_enabled": false},
											"cerebrospinal_fluid":
												{
												"albedo_color": Color(0.4, 0.7, 1.0, 0.7),
												"roughness": 0.3,
												"transparency": BaseMaterial3D.TRANSPARENCY_ALPHA
												},
												"blood_vessel":
													{"albedo_color": Color(0.9, 0.1, 0.1), "roughness": 0.5, "subsurf_scatter_enabled": false},
