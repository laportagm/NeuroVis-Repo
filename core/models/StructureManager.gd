...
enum RelationshipType {
    FUNCTIONAL,     # Structures that work together functionally
    ANATOMICAL,     # Structures physically connected/adjacent
    DEVELOPMENTAL,  # Structures from same developmental origin
    PATHOLOGICAL    # Structures affected by same pathologies
}

## Educational highlight types
enum HighlightType {
    PRIMARY,        # Main educational focus
    SECONDARY,      # Related to primary - functional connection
    RELATED,        # Anatomically connected
    PATHWAY         # Part of neural pathway
}
