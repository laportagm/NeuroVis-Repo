...
func preload_resources(resource_paths: Array, group_name: String = DEFAULT_GROUP) -> void:
	...
	print("[ResourceManager] Preloading %d resources in group '%s'" % [resource_paths.size(), group_name])
	...
	print("[ResourceManager] Preloaded %d/%d resources in group '%s'" % [group_loaded_count, group_total, group_name])
...
func load_resource_async(resource_path: String, callback: Callable = Callable()) -> bool:
	...
	if load_status == OK:
		...
		push_error("[ResourceManager] Failed to start async load: %s (Error: %d)" % [resource_path, load_status])
		...
		match load_status:
			ResourceLoader.THREAD_LOAD_LOADED:
				...
				if resource:
					...
					push_error("[ResourceManager] Async load completed but resource is null: %s" % resource_path)
			...
			ResourceLoader.THREAD_LOAD_IN_PROGRESS:
				...
				var progress = []
				var progress_value = ResourceLoader.load_threaded_get_status(resource_path, progress)
