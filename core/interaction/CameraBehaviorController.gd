...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		...
		elif is_panning or (Input.is_key_pressed(KEY_SHIFT) and event.button_mask == MOUSE_BUTTON_MASK_LEFT):
			var pan_delta = Vector3(delta.x * PAN_SPEED * camera_distance, -delta.y * PAN_SPEED * camera_distance, 0.0)
		...
		var pan_delta = Vector3(event.delta.x * PAN_SPEED * camera_distance * 0.5, -event.delta.y * PAN_SPEED * camera_distance * 0.5, 0.0)
		var cam_transform = camera.global_transform
		pan_velocity += cam_transform.basis.x * pan_delta.x + cam_transform.basis.y * pan_delta.y
...
func set_view_preset(preset: String) -> void:
	match preset:
		"front": target_rotation = Vector2(0, 0)
		"right": target_rotation = Vector2(0, -PI/2)
