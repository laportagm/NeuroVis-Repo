...
enum CameraMode {
	...
	PAN,            # Pan/move the camera
	ZOOM,           # Zoom in/out
	FLY,            # Free fly mode
	FOCUS,          # Focus on structure
	LOCKED          # Locked (no movement)
}
...
func _handle_pan_input(delta: float) -> void:
	...
	if input_dir != Vector3.ZERO:
		var move_vec = camera.global_transform.basis * input_dir.normalized() * movement_speed * delta * _camera_distance * 0.5
...
func _handle_fly_input(delta: float) -> void:
	...
	# Apply movement
	if input_dir != Vector3.ZERO:
		var move_vec = camera.global_transform.basis * input_dir.normalized() * movement_speed * delta * 5.0
		camera.global_position += move_vec
...
func _transition_to_transform(target_transform: Transform3D) -> void:
	...
	_current_tween.tween_property(camera, "global_position", target_transform.origin, transition_time)
