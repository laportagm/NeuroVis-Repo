...
# === ENUMS ===
enum ComparisonType {
    STRUCTURE_FUNCTION,  # Compare structure and function
    SPECIES_DIFFERENCE,  # Compare between species
    PATHOLOGICAL,        # Compare normal vs pathological
    DEVELOPMENTAL,       # Compare developmental stages
    IMAGING_MODALITY     # Compare different imaging techniques
}

enum AnnotationStyle {
    SIMPLE_LABELS,       # Basic text labels
    DETAILED_CALLOUTS,   # Detailed callouts with arrows
    COLOR_HIGHLIGHTING,  # Structures highlighted with color
    INTERACTIVE_MARKERS  # Clickable interactive markers
}
...
func get_available_comparison_types() -> Array:
    ...
    for type_value in ComparisonType.values():
        types.append({
            "id": type_value,
            "name": ComparisonType.keys()[type_value]
        })
    return types
...
func _validate_comparison_entry(entry: Dictionary) -> bool:
    ...
    var required_fields = [
        "title", "type", "primary_model", "secondary_model", 
        "key_differences", "educational_focus", "key_structures"
    ]
...
func _load_comparison_models(comparison: Dictionary) -> bool:
    ...
    _loaded_comparison_models = {
        "primary": comparison.resource_path_primary,
        "secondary": comparison.resource_path_secondary
    }
...
func _generate_visualization_tips(comparison: Dictionary) -> Array:
    ...
    match comparison.type:
        ComparisonType.STRUCTURE_FUNCTION:
            tips.append("Observe how structural differences correspond to functional specialization")
