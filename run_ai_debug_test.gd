...
func _ready():
	...
	if gemini_ai:
		...
		if gemini_ai.has_method("check_setup_status"):
			print("   Setup status: " + ("Complete" if gemini_ai.check_setup_status() else "Not configured"))
	else:
	...
	if debug_cmd.has_method("has_command"):
		var commands_to_check = ["ai_status", "ai_provider", "ai_test", "ai_gemini_status", "ai_gemini_setup", "ai_gemini_reset"]
	...
	if ai_assistant:
		...
		var provider_names = ["OPENAI_GPT", "ANTHROPIC_CLAUDE", "GOOGLE_GEMINI", "GEMINI_USER", "MOCK_RESPONSES"]
