...
## @returns: Dictionary with section and item IDs
	var safe_autoload_script = preprepreload("res://ui/components/core/SafeAutoloadAccess.gd")
	...
	if not section_node:
		var section_script = preprepreload("res://ui/components/navigation/NavigationSection.gd")
		if section_script:
