...
func run_test() -> bool:
	...
	if file:
		...
		if parse_result == OK:
			...
			framework.assert_true(data.structures.size() > 0, "Structures array should not be empty")
	...
	if kb.is_loaded:
		...
		if structure_ids.size() > 0:
			...
			framework.assert_true(structure_data.has("displayName"), "Structure should have displayName")
			framework.assert_true(structure_data.has("shortDescription"), "Structure should have shortDescription")
