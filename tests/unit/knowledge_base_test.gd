...
func _run_tests() -> void:
	...
	for structure_id in structure_ids:
		...
		if not structure.has("id") or not structure.has("displayName") or not structure.has("shortDescription") or not structure.has("functions"):
	...
	if test_structure.id != test_structure_id:
		_report_failure("Structure ID mismatch. Expected: " + test_structure_id + ", Got: " + test_structure.id)
		return
