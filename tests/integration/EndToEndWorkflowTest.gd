...
func run_test() -> bool:
	...
	framework.assert_true(model_names.has("Internal Structures"), "Should have Internal Structures model")
	...
	framework.assert_true(structure_data.has("shortDescription"), "Structure should have description")
	framework.assert_true(structure_data.has("functions"), "Structure should have functions")
	framework.assert_true(structure_data.functions.size() > 0, "Functions array should not be empty")
	...
	framework.assert_false(model_switcher.is_model_visible("Half Brain"), "Half Brain should be hidden")
	framework.assert_false(model_switcher.is_model_visible("Internal Structures"), "Internal Structures should be hidden")
	framework.assert_false(model_switcher.is_model_visible("Brainstem"), "Brainstem should be hidden")
	
	# Test selective visibility
	model_switcher.toggle_model_visibility("Half Brain")
	framework.assert_true(model_switcher.is_model_visible("Half Brain"), "Half Brain should be visible after toggle")
	framework.assert_false(model_switcher.is_model_visible("Internal Structures"), "Internal Structures should remain hidden")
	
	# Test show all
	model_switcher.show_all_models()
	framework.assert_true(model_switcher.is_model_visible("Half Brain"), "Half Brain should be visible")
	framework.assert_true(model_switcher.is_model_visible("Internal Structures"), "Internal Structures should be visible")
	framework.assert_true(model_switcher.is_model_visible("Brainstem"), "Brainstem should be visible")
	...
	framework.assert_execution_time_under(lookup_time, 50.0, "100 structure lookups should complete under 50ms")
	...
	framework.assert_execution_time_under(toggle_time, 25.0, "50 visibility toggles should complete under 25ms")
	...
	framework.assert_true(invalid_structure.is_empty(), "Invalid structure lookup should return empty")
	...
	framework.assert_equal(initial_model_count, model_switcher.get_model_names().size(), "Invalid toggle should not affect model count")
	
	# Test null model registration
	model_switcher.register_model(null, "NullModel")
	framework.assert_equal(initial_model_count, model_switcher.get_model_names().size(), "Null model should not be registered")
	...
	return test1_result and test2_result and test3_result and test4_result and test5_result and test6_result