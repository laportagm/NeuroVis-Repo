...
func test_dialog_creation() -> void:
	...
	assert(dialog.current_state == dialog.SetupState.INITIAL, "Dialog should start in INITIAL state")
...
func test_state_transitions() -> void:
	...
	assert(dialog.current_state == dialog.SetupState.GOOGLE_CONSOLE, "Dialog should transition to GOOGLE_CONSOLE state")
	...
	dialog._on_next_button_pressed()
	assert(dialog.current_state == dialog.SetupState.RETURN_WITH_KEY, "Dialog should transition to RETURN_WITH_KEY state")
	...
	assert(dialog.current_state == dialog.SetupState.GOOGLE_CONSOLE, "Dialog should transition back to GOOGLE_CONSOLE state")
	print("[TEST] Back button navigation successful")
	
	# Go forward again
	dialog._on_next_button_pressed()
	assert(dialog.current_state == dialog.SetupState.RETURN_WITH_KEY, "Dialog should transition to RETURN_WITH_KEY state again")
...
func test_config_saving() -> void:
	...
	assert(gemini_service.current_model == dialog.DEFAULT_MODEL, "Model should use educational default")
	assert(abs(gemini_service.temperature - dialog.DEFAULT_TEMPERATURE) < 0.01, "Temperature should use educational default")
	assert(gemini_service.max_output_tokens == dialog.DEFAULT_MAX_TOKENS, "Max tokens should use educational default")
	...
	for category in dialog.DEFAULT_SAFETY_SETTINGS:
		...
		assert(gemini_service.safety_settings[category] == dialog.DEFAULT_SAFETY_SETTINGS[category], 
...
func _on_setup_completed(successful: bool, api_key: String) -> void:
	print("[TEST] Setup completed signal received. Success: ", successful, ", API key length: ", api_key.length())
