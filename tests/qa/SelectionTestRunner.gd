...
func run_selection_test(mode: String = "full", target_structure: String = "") -> void:
    ...
    match mode:
        "full":
            print("[SelectionTestRunner] Starting FULL reliability test (all structures, all angles)")
        ...
        "structure":
            if target_structure.is_empty():
                print("[SelectionTestRunner] ERROR: Structure name required for single structure test")
                _cleanup_test()
                return
...
func get_test_status() -> void:
    ...
    print("Overall Progress: %.1f%% (%d/%d tests)" % [
        progress["percentage"],
        progress["completed"],
        progress["total"]
    ])
...
func _register_debug_commands() -> void:
    ...
    # Main test commands
    DebugCmd.register_command("qa_test", _cmd_run_test, 
        "Run selection reliability test [mode: full|quick|structure] [structure_name]")
    
    DebugCmd.register_command("qa_stop", _cmd_stop_test,
        "Stop the current selection test")
    
    DebugCmd.register_command("qa_status", _cmd_test_status,
        "Show current test progress")
    
    # Analysis commands
    DebugCmd.register_command("qa_analyze", _cmd_analyze_selection,
        "Analyze current selection system performance")
    
    DebugCmd.register_command("qa_bounds", _cmd_show_bounds,
        "Show bounds visualization for structures")
    
    DebugCmd.register_command("qa_simulate", _cmd_simulate_clicks,
        "Simulate clicks on a structure [structure_name] [count]")
    
    # Performance validation
    DebugCmd.register_command("qa_perf", _cmd_performance_test,
        "Run performance validation test")
    
    DebugCmd.register_command("qa_perf_status", _cmd_performance_status,
...
func _run_quick_test() -> void:
    ...
    if _test_instance:
        ...
        _test_instance.set_test_configuration("quick", [
            "Hippocampus", "Thalamus", "Cerebellum", 
            "Frontal_Lobe", "Brainstem"
        ])
        # Start test
        _test_instance.start_test()
...
func _cmd_performance_test(_args: String = "") -> void:
    ...
    var PerfValidator = load("res://tests/qa/SelectionPerformanceValidator.gd")
...
func _on_perf_test_completed(results: Dictionary) -> void:
    ...
    print("Memory Usage: %.1f MB" % ((results.get("memory_end", 0) - results.get("memory_start", 0)) / 1048576.0))
