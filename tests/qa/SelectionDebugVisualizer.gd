...
func _create_bounds_visualization(mesh_instance: MeshInstance3D, structure_name: String) -> void:
    ...
    mesh_instance_3d.transform = global_transform
    mesh_instance_3d.position = global_transform * aabb.get_center()
    
    # Create transparent material
    var material = StandardMaterial3D.new()
    material.albedo_color = BOUNDS_COLOR
...
func _create_collision_visualization(collision_shape: CollisionShape3D) -> void:
    ...
    if collision_shape.shape is BoxShape3D:
        ...
        var aabb = collision_shape.get_parent().get_aabb() if collision_shape.get_parent() is MeshInstance3D else AABB()
        var box_mesh = BoxMesh.new()
        box_mesh.size = aabb.size
        mesh_instance.mesh = box_mesh
