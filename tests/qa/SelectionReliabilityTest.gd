...
# FIXED: Orphaned code - var cameras = _camera_positions if not _camera_positions.is_empty() else CAMERA_POSITIONS
...
# FIXED: Orphaned code - var structures_2 = (
_structures_to_test if not _structures_to_test.is_empty() else STRUCTURES_TO_TEST
)
# FIXED: Orphaned code - var cameras_2 = _camera_positions if not _camera_positions.is_empty() else CAMERA_POSITIONS
...
# FIXED: Orphaned code - var bounds = Rect2()
# FIXED: Orphaned code - var found_mesh = false

# Search for the structure mesh
var meshes = _find_meshes_recursive(child, structure_name)
# FIXED: Orphaned code - var screen_rect = _calculate_screen_bounds(mesh)
# FIXED: Orphaned code - var meshes_2: Array[MeshInstance3D] = []

var node_name = node.name.to_lower()
# FIXED: Orphaned code - var search_name = structure_name.to_lower().replace("_", " ")

# FIXED: Orphaned code - var aabb = mesh.get_aabb()
# FIXED: Orphaned code - var transform = mesh.global_transform
...
# FIXED: Orphaned code - var min_pos = screen_points[0]
...
# FIXED: Orphaned code - var progress = get_test_progress()
...
# FIXED: Orphaned code - var edge_threshold = 5.0  # pixels

var dist_to_left = abs(position.x - bounds.position.x)
# FIXED: Orphaned code - var dist_to_right = abs(position.x - (bounds.position.x + bounds.size.x))
# FIXED: Orphaned code - var dist_to_top = abs(position.y - bounds.position.y)
# FIXED: Orphaned code - var dist_to_bottom = abs(position.y - (bounds.position.y + bounds.size.y))

# FIXED: Orphaned code - var reps_2 = _repetitions if _repetitions > 0 else REPETITIONS_PER_STRUCTURE
...
# FIXED: Orphaned code - var structures_3 = (
_structures_to_test
...
var report = (
"""# NeuroVis Selection Reliability Test Report
Generated: %s
...
# FIXED: Orphaned code - var critical_count = 0
var patterns = _analyze_failure_patterns(
_test_results[struct_data["name"]]["failure_coordinates"]
)
# FIXED: Orphaned code - var high_count = 0
var score = _test_results[struct_data["name"]]["difficulty_score"]
var patterns_2: Array[String] = []

var camera_failures = {}
# FIXED: Orphaned code - var zoom_failures = {}
# FIXED: Orphaned code - var misselection_targets = {}

# FIXED: Orphaned code - var camera = failure.get("camera", "unknown")
# FIXED: Orphaned code - var zoom = str(failure.get("zoom", "unknown"))
# FIXED: Orphaned code - var selected_instead = failure.get("selected_instead", "nothing")
# FIXED: Orphaned code - var total = 0
...
# FIXED: Orphaned code - var std_dev_difficulty = sqrt(variance_difficulty)
...
# FIXED: Orphaned code - var count = 0
var timestamp = Time.get_datetime_string_from_system().replace(" ", "_").replace(":", "-")
# FIXED: Orphaned code - var file_path = "user://selection_reliability_report_%s.md" % timestamp

var file = FileAccess.open(file_path, FileAccess.WRITE)
# FIXED: Orphaned code - var project_file_path = "res://test_reports/selection_reliability_%s.md" % timestamp
var project_file = FileAccess.open(project_file_path, FileAccess.WRITE)

# FIXED: Orphaned code - var _main_scene: Node3D
...
# FIXED: Orphaned code - var _current_test_data: Dictionary = {}
...
func initialize(
	main_scene: Node3D, selection_manager: Node, camera_controller: Node, camera: Camera3D
	) -> bool:
...
func set_test_configuration(mode: String, structures: Array = []) -> void:
	...
	match mode:
		"quick":
			...
			else ["Hippocampus", "Thalamus", "Cerebellum", "Frontal_Lobe", "Brainstem"]
			)
			_camera_positions = [
			{"name": "front", "rotation": Vector3(0, 0, 0), "distance": 5.0},
			{"name": "right", "rotation": Vector3(0, PI / 2, 0), "distance": 5.0},
...
"current_structure":
	...
	"current_camera":
		cameras[_current_camera_index]["name"] if _current_camera_index < cameras.size() else "",
...
or dist_to_right < edge_threshold
or dist_to_top < edge_threshold
or dist_to_bottom < edge_threshold
)
...
if not _structures_to_test.is_empty()
else STRUCTURES_TO_TEST
)
...
if results["total_attempts"] > 0:
	success_rate = (
	float(results["successful_selections"]) / float(results["total_attempts"]) * 100.0
	)
...
return (
structures_done * CAMERA_POSITIONS.size() * ZOOM_LEVELS.size() * REPETITIONS_PER_STRUCTURE
...
if cam_data["attempts"] > 0:
	cam_success_rate = (
	float(cam_data["successes"]) / float(cam_data["attempts"]) * 100.0
	)
	report += (
	"  - %s: %.1f%% (%d/%d)\n"
...
if zoom_data["attempts"] > 0:
	zoom_success_rate = (
	float(zoom_data["successes"]) / float(zoom_data["attempts"]) * 100.0
	)
	report += (
	"  - %s units: %.1f%% (%d/%d)\n"
...
for struct_data in sorted_structures:
	if _test_results[struct_data["name"]]["difficulty_score"] >= 70:
		...
		report += (
		"1. **%s** - %.1f%% success rate\n"
		% [struct_data["name"], _test_results[struct_data["name"]]["success_rate"]]
		)
...
if not misselection_targets.has(selected_instead):
	...
	for camera in camera_failures:
		if camera_failures[camera] >= 3:
			patterns.append(
			"High failure rate from %s view (%d failures)" % [camera, camera_failures[camera]]
			)
			...
			for zoom in zoom_failures:
				if zoom_failures[zoom] >= 3:
					patterns.append(
					(
					"Difficult to select at %s unit distance (%d failures)"
					% [zoom, zoom_failures[zoom]]
					)
					)
					...
					for target in misselection_targets:
						if target != "nothing" and misselection_targets[target] >= 2:
							patterns.append(
							"Often misselects '%s' instead (%d times)" % [target, misselection_targets[target]]
							)
...
func _perform_selection_test(
	structure_name: String, bounds: Rect2, camera_name: String, zoom: float
	) -> void:
