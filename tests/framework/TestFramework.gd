...
func end_test() -> bool:
	...
	if success:
		...
		print("❌ Test failed: %s (%d/%d assertions failed)" % [current_test_name, assertions_failed, assertions_passed + assertions_failed])
...
func assert_not_equal(not_expected, actual, message: String = ""):
	if not_expected != actual:
		...
		assertions_failed += 1
		print("  ✗ Assertion failed: Expected not '%s' but got '%s'. %s" % [not_expected, actual, message])
...
func assert_vector3_equal(expected: Vector3, actual: Vector3, tolerance: float = 0.001, message: String = ""):
	if expected.distance_to(actual) <= tolerance:
		assertions_passed += 1
		print("  ✓ Vector3 assertion passed: %s" % message if message else "vector equality")
	else:
		assertions_failed += 1
		print("  ✗ Vector3 assertion failed: Expected '%s' but got '%s' (tolerance: %f). %s" % [expected, actual, tolerance, message])
...
func assert_execution_time_under(execution_time_ms: float, max_time_ms: float, message: String = ""):
	if execution_time_ms <= max_time_ms:
		assertions_passed += 1
		print("  ✓ Performance assertion passed: %dms <= %dms" % [execution_time_ms, max_time_ms])
	else:
		assertions_failed += 1
		print("  ✗ Performance assertion failed: %dms > %dms. %s" % [execution_time_ms, max_time_ms, message])

# Mock system for testing
class MockNode3D extends Node3D:
	var method_calls: Dictionary = {}
	
	func mock_method_call(method_name: String, args: Array = []):
		if not method_calls.has(method_name):
			method_calls[method_name] = []
		method_calls[method_name].append(args)
	
	func was_method_called(method_name: String) -> bool:
		return method_calls.has(method_name)
	
	func get_method_call_count(method_name: String) -> int:
		if method_calls.has(method_name):
			return method_calls[method_name].size()
		return 0
