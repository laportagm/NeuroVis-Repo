...
func run_test() -> bool:
	...
	framework.assert_true(loadable_scenes == scene_files.size(), "All scenes should load successfully")
	...
	if scene_load_times.size() > 0:
		...
		framework.assert_execution_time_under(avg_load_time, 1000.0, "Average scene load time should be reasonable")
	...
	var ui_scenes = [
		"res://scenes/ui_info_panel.tscn",
		"res://scenes/model_control_panel.tscn"
	]
	...
	for ui_scene_path in ui_scenes:
		if FileAccess.file_exists(ui_scene_path):
			...
			if ui_scene:
				...
				if ui_instance:
					loaded_ui_scenes += 1
					print("  ✓ %s: %s with %d children" % [ui_scene_path.get_file(), ui_instance.get_class(), ui_instance.get_child_count()])
