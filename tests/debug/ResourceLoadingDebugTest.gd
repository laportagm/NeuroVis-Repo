...
func run_test() -> bool:
	...
	if model_load_times.size() > 0:
		...
		framework.assert_execution_time_under(avg_load_time, 2000.0, "Model loading should be reasonable")
	...
	framework.assert_true(compilation_errors < loaded_scripts, "Most scripts should compile successfully")
	...
	var initial_resource_count = ResourceLoader.get_recognized_extensions_for_type("Resource").size()
	...
	while file_name != "":
		if not file_name.begins_with("."):
			...
			if dir.current_is_dir():
				...
				elif file_name.ends_with(".json") or file_name.ends_with(".txt") or file_name.ends_with(".csv"):
