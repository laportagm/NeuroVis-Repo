...
func test_ai_status():
	...
	if ai_service:
		...
		if status.has("gemini_status"):
			...
			print("[color=#00CED1]Gemini rate limit: %d/%d (resets in %ds)[/color]" % [
...
func test_ai_gemini_status():
	...
	if gemini:
		...
		var setup_complete = gemini.check_setup_status() if gemini.has_method("check_setup_status") else false
		...
		if gemini.has_method("get_rate_limit_status"):
			...
			print("[color=#00CED1]Rate limit: %d/%d[/color]" % [rate.get("used", 0), rate.get("limit", 0)])
			if rate.get("reset_in", 0) > 0:
				print("[color=#00CED1]Resets in: %d seconds[/color]" % rate.reset_in)
		
		print("[color=#00CED1]API key: %s[/color]" % ("Configured" if setup_complete else "Not configured"))
	else:
...
func test_ai_provider():
	...
	if ai_service:
		...
		print("[color=#FF6B6B]ERROR: Unknown provider. Available: [\"openai\", \"claude\", \"gemini\", \"gemini_user\", \"mock\"][/color]")
	else:
		print("[color=#FF6B6B]ERROR: AI Assistant service not found[/color]")
