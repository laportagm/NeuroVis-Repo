...
func run_single_test(test_name: String) -> bool:
	...
	if test_name not in ["idle_fps", "model_loading", "rotation_fps", "selection_performance", "memory_usage"]:
...
func _start_next_test() -> void:
	...
	benchmark_progress.emit(
		1.0 - float(_test_queue.size() + 1) / 5.0,
		"Running test: " + _current_test
	)
...
func _calculate_summary() -> void:
	...
	# Model loading time
	if _results.tests.has("model_loading") and _results.tests.model_loading.has("average_load_time"):
	...
	if _results.tests.has("selection_performance") and _results.tests.selection_performance.has("average_selection_time"):
		summary["selection_response_time"] = _results.tests.selection_performance.average_selection_time
...
func _take_common_measurements() -> void:
	...
	# Memory usage
	var memory_info = {}
	memory_info["static_memory"] = Performance.get_monitor(Performance.MEMORY_STATIC)
	memory_info["dynamic_memory"] = Performance.get_monitor(Performance.MEMORY_DYNAMIC)
	memory_info["total_memory"] = memory_info.static_memory + memory_info.dynamic_memory
	...
	if detailed_logging:
		print("[RenderingBenchmark] Measurement - FPS: %f, Memory: %f MB, Draw Calls: %d" % 
			[
				_fps_samples[-1] if not _fps_samples.is_empty() else 0, 
				_memory_samples[-1].total_memory / 1048576.0 if not _memory_samples.is_empty() else 0,
...
func _find_selection_manager() -> Node:
	...
	var selection_manager_paths = [
		...
		"/root/MultiStructureSelectionManager"
	]
