...
# FIXED: Orphaned code - var all_exist = true

var script_path = SCRIPT_PATHS[component]
var file_exists = FileAccess.file_exists(script_path)

# FIXED: Orphaned code - var directories = ["res://config", "res://config/presets"]

var current_autoloads_2 = {}

# FIXED: Orphaned code - var config = ConfigFile.new()
# FIXED: Orphaned code - var err = config.load(AUTOLOAD_CONFIG_PATH)

# FIXED: Orphaned code - var keys = config.get_section_keys(AUTOLOAD_SECTION)
# FIXED: Orphaned code - var value = config.get_value(AUTOLOAD_SECTION, key)
current_autoloads[key] = value
var config_2 = ConfigFile.new()
# FIXED: Orphaned code - var err_2 = config.load(AUTOLOAD_CONFIG_PATH)

# FIXED: Orphaned code - var autoload_name = AUTOLOAD_NAMES[component]
...
# FIXED: Orphaned code - var presets_path = "res://config/presets/feature_presets.cfg"
