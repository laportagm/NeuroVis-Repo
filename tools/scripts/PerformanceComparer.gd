...
func compare_performance():
	...
	for metric in metrics_to_compare:
		if previous_results.has(metric) and current_results.has(metric):
			...
			var change_percent = ((curr_value - prev_value) / prev_value) * 100.0 if prev_value != 0 else 0
			...
			print("  %s %s: %.4f → %.4f (%.1f%%)" % [icon, metric, prev_value, curr_value, change_percent])
