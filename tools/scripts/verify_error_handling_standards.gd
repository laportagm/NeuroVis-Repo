...
const VALID_PREFIXES = [
	"[UI]", "[Selection]", "[Camera]", "[Model]", "[Knowledge]", "[AI]",
	"[Debug]", "[System]", "[Resource]", "[Scene]", "[Test]", "[Tool]",
	"[Navigation]", "[Panel]", "[Component]", "[Theme]", "[Education]",
	"[Analytics]", "[Interaction]", "[Visualization]", "[Performance]",
	"[Accessibility]", "[State]", "[Event]", "[Feature]", "[Service]"
]
...
func check_file(file_path: String) -> void:
	...
	for line in content.split("\n"):
		...
		if line.contains("push_error") or line.contains("push_warning") or \
		   (line.contains("print(") and (line.contains("Error") or line.contains("Warning"))):
...
func print_summary(duration: float) -> void:
	...
	print("Compliant files: %d (%.1f%%)" % [compliant_files, 
		(compliant_files * 100.0) / max(total_files_checked, 1)])
	print("Non-compliant files: %d" % non_compliant_files.size())
	
	print("\n=== Issues by Type ===\n")
	for issue_type in issues_by_type:
		if issues_by_type[issue_type] > 0:
			print("%s: %d" % [issue_type.capitalize().replace("_", " "), 
	...
	if issues_by_type["missing_prefix"] > 0:
		print("• Add component prefixes to %d error/warning messages" % issues_by_type["missing_prefix"])
	if issues_by_type["non_standard_concat"] > 0:
		print("• Replace string concatenation with proper formatting in %d messages" % issues_by_type["non_standard_concat"])
	if issues_by_type["using_printerr"] > 0:
		print("• Replace printerr() with push_error() in %d locations" % issues_by_type["using_printerr"])
	if issues_by_type["invalid_prefix"] > 0:
		print("• Update %d messages to use valid component prefixes" % issues_by_type["invalid_prefix"])
