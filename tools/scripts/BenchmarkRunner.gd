...
func run_rendering_benchmarks():
	...
	print("  Transform Calculations: %.6fms avg (%d iterations)" % [transform_avg, transform_iterations])
...
func run_memory_benchmarks():
	...
	print("  Large Object Allocation: %dms (%d objects)" % [memory_alloc_time, array_count * array_size])
