...
func _ready():
	...
	if main_scene and main_scene.has_method("_show_gemini_setup_dialog"):
		...
		print("   [color=#%s]ERROR: Cannot open setup dialog from here. Please restart the app.[/color]" % error_color)
	...
	print("   Result:")
	if gemini:
		print("   [color=#%s]Gemini settings reset. Run ai_gemini_setup to reconfigure.[/color]" % success_color)
		print("   ✅ Command would work correctly")
	else:
		print("   [color=#%s]ERROR: GeminiAI service not found[/color]" % error_color)
		print("   ⚠️ Command would show error")
