...
# Signals
	...
	var formatted = (
		...
		% [UIThemeManager.FONT_SIZE_MEDIUM, UIThemeManager.TEXT_PRIMARY.to_html(), text]
	)
	...
	return formatted
		...
		UIThemeManager.apply_modern_label(
			placeholder, UIThemeManager.FONT_SIZE_SMALL, UIThemeManager.TEXT_TERTIARY
		)
	...
	for i in range(functions_array.size()):
		...
		UIThemeManager.animate_entrance(
			function_card, i * 0.1, UIThemeManager.ANIM_DURATION_STANDARD, "slide_up"
		)
	...
	UIThemeManager.apply_modern_label(
		icon_label, UIThemeManager.FONT_SIZE_LARGE, UIThemeManager.ACCENT_CYAN
	)
	...
	UIThemeManager.apply_modern_label(
		text_label, UIThemeManager.FONT_SIZE_SMALL, UIThemeManager.TEXT_PRIMARY
	)
	container.add_child(text_label)

	# Add subtle hover effect
	card.mouse_entered.connect(
		func(): UIThemeManager.animate_hover_glow(card, UIThemeManager.ACCENT_BLUE, 0.1)
	)
	...
	var progress_data = UIThemeManager.create_learning_progress_indicator(
		0.0, total_items, completed_items
	)
	...
	if progress_label:
		...
		UIThemeManager.apply_modern_label(
			progress_label, UIThemeManager.FONT_SIZE_SMALL, progress_data["color"]
		)
	...
	if difficulty_indicator:
		...
		UIThemeManager.apply_modern_label(
			difficulty_indicator, UIThemeManager.FONT_SIZE_SMALL, difficulty_data["color"]
		)
...
func clear_data() -> void:
	...
	if description_text:
		description_text.text = (
			"[center][font_size=%d][color=%s][i]Select a brain structure to explore detailed information, functions, and learning resources.[/i][/color][/font_size][/center]"
			% [UIThemeManager.FONT_SIZE_MEDIUM, UIThemeManager.TEXT_TERTIARY.to_html()]
		)
...
func show_search_section() -> void:
	...
	UIThemeManager.animate_entrance(
		search_container, 0.0, UIThemeManager.ANIM_DURATION_FAST, "slide_up"
	)
...
func _apply_enhanced_styling() -> void:
	...
	# Header styling
	UIThemeManager.apply_modern_label(
		structure_name_label, UIThemeManager.FONT_SIZE_H2, UIThemeManager.TEXT_PRIMARY, "heading"
	)
	UIThemeManager.apply_modern_label(
		structure_id_badge, UIThemeManager.FONT_SIZE_TINY, UIThemeManager.TEXT_ACCENT, "badge"
	)
	...
	# Section headers
	UIThemeManager.apply_modern_label(
		description_header, UIThemeManager.FONT_SIZE_H3, UIThemeManager.TEXT_ACCENT, "subheading"
	)
	UIThemeManager.apply_modern_label(
		functions_header, UIThemeManager.FONT_SIZE_H3, UIThemeManager.TEXT_ACCENT, "subheading"
	)
	UIThemeManager.apply_modern_label(
		related_header, UIThemeManager.FONT_SIZE_H3, UIThemeManager.TEXT_ACCENT, "subheading"
	)
	UIThemeManager.apply_modern_label(
		learning_header, UIThemeManager.FONT_SIZE_H3, UIThemeManager.TEXT_ACCENT, "subheading"
	)

	# Content styling
	UIThemeManager.apply_rich_text_styling(
		description_text, UIThemeManager.FONT_SIZE_MEDIUM, "description"
	)
	...
	UIThemeManager.apply_modern_label(
		...
		UIThemeManager.TEXT_SECONDARY,
		"caption"
	)
	UIThemeManager.apply_modern_label(
		reading_time_label, UIThemeManager.FONT_SIZE_SMALL, UIThemeManager.TEXT_SECONDARY, "caption"
	)
	UIThemeManager.apply_modern_label(
		progress_label, UIThemeManager.FONT_SIZE_SMALL, UIThemeManager.TEXT_PRIMARY
	)
...
func _animate_enhanced_entrance() -> void:
	...
	get_tree().create_timer(0.1).timeout.connect(
		func():
			if header_container:
				UIThemeManager.animate_entrance(
					header_container, 0.0, UIThemeManager.ANIM_DURATION_FAST, "fade_scale"
				)
	)
	...
	get_tree().create_timer(0.2).timeout.connect(
		func():
			if description_section:
				UIThemeManager.animate_entrance(
					description_section, 0.0, UIThemeManager.ANIM_DURATION_FAST, "slide_up"
				)
	)

	get_tree().create_timer(0.3).timeout.connect(func(): is_animating = false)
...
func _on_close_pressed() -> void:
	...
	get_tree().create_timer(UIThemeManager.ANIM_DURATION_FAST).timeout.connect(
		func(): panel_closed.emit()
	)
