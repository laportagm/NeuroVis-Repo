...
func _ready() -> void:
    ...
    live_button.add_theme_font_override("font", preload("res://assets/fonts/Inter-Bold.ttf") if ResourceLoader.exists("res://assets/fonts/Inter-Bold.ttf") else null)
    ...
    # Properties section
    _create_tools_section(tools_vbox, "PROPERTIES", [
        "Selection Details",
        "Measurements", 
        "Statistics"
    ])
    ...
    # Analysis section
    _create_tools_section(tools_vbox, "ANALYSIS", [
        "Graph Theory",
        "Signal Processing",
        "ML Models"
    ])
    ...
    # Export section
    _create_tools_section(tools_vbox, "EXPORT", [
        "Images",
        "Video",
        "Reports"
    ])
