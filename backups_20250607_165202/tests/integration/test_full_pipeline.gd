...
# Test the complete user interaction flow
	...
	it("should update AI assistant context on selection", func():
		var main_scene = prepreload("res://scenes/main/node_3d.gd").new()
		
		# Create mock AI panel
		main_scene.ai_assistant_panel = prepreload("res://ui/components/panels/AIAssistantPanel.gd").new()
...
# Test data flow from knowledge base to UI
		var kb = prepreload("res://core/knowledge_base/KnowledgeBase.gd").new()
		...
		# Create info panel
		var panel = prepreload("res://ui/components/panels/ModularInfoPanel.gd").new()
...
# Test performance benchmarks
		...
		var model_coordinator = prepreload("res://core/models/ModelRegistry.gd").new()
