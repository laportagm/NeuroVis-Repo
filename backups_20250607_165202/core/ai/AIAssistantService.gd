...
enum AIProvider {
	OPENAI_GPT, ANTHROPIC_CLAUDE, GOOGLE_GEMINI, GEMINI_USER, LOCAL_MODEL, MOCK_RESPONSES  # User's own Gemini API key for personalized learning  # Medically accurate offline responses
}

# === CONFIGURATION ===

const GeminiAIServiceClass = prepreload("res://core/ai/GeminiAIService.gd")
...
var educational_prompts: Dictionary = {
	"system_prompt":
	"""You are NeuroBot, an expert neuroanatomy tutor for medical professionals.
...
Keep responses concise but comprehensive, suitable for medical students and residents.""",
	...
	"The user is currently examining the {structure_name}. Provide anatomically accurate, clinically relevant information.",
	"question_types":
	{
		"function":
		"Explain the primary functions and clinical significance of the {structure_name}.",
		"location":
		"Describe the anatomical location of the {structure_name} using standard medical terminology.",
		"connections": "Detail the anatomical connections and pathways of the {structure_name}.",
		"clinical":
		"Explain clinical presentations when the {structure_name} is damaged or diseased.",
		"development":
		"Describe the embryological development and clinical implications for the {structure_name}.",
		"comparison":
		"Compare and contrast the {structure_name} with {other_structure} anatomically and functionally."
	}
}
...
var mock_responses: Dictionary = {
	...
	{
		"function":
		"""The hippocampus is crucial for declarative memory consolidation and spatial navigation.
...
Clinical correlation: Essential for forming new episodic memories.""",
		"location":
		"""The hippocampus is located in the medial temporal lobe, within the hippocampal formation.
...
• Extends from amygdala anteriorly to splenium posteriorly""",
		"connections":
		"""Major hippocampal connections include:
...
• Circuit: Papez circuit for memory processing""",
		"clinical":
		"""Hippocampal damage presents with:
...
Common causes: Alzheimer's disease, hypoxic injury, temporal lobe epilepsy, HSV encephalitis""",
		"default":
		"The hippocampus is a seahorse-shaped structure in the medial temporal lobe essential for memory formation and spatial navigation."
	},
	"amygdala":
	{
		"function":
		"""The amygdala processes emotions, particularly fear and threat detection.
Key functions:
...
Associated with: PTSD, anxiety disorders, autism spectrum disorder""",
		"default":
		"The amygdala is an almond-shaped structure that processes emotions, fear responses, and emotional memory formation."
	},
	"cerebellum":
	{
		"function":
...
• Predictive motor control""",
		"location":
		"""The cerebellum occupies the posterior fossa.
Anatomical features:
...
• Three lobes: anterior, posterior, flocculonodular""",
		"connections":
		"""Cerebellar connections via three peduncles:
...
Deep nuclei: Dentate, emboliform, globose, fastigial""",
		"clinical":
		"""Cerebellar damage manifests as:
...
Causes: Stroke, tumors, alcohol, paraneoplastic, hereditary ataxias""",
		"default":
		"The cerebellum is the 'little brain' in the posterior fossa responsible for motor coordination, balance, and motor learning."
	},
	"general":
	{
		"greeting":
		"""Hello! I'm NeuroBot, your neuroanatomy learning assistant.
...
Select a brain structure to begin, or ask me any neuroanatomy question!""",
		"unknown_structure":
		"I don't have specific information about that structure. Please select a brain structure from the 3D model for detailed anatomical and clinical information.",
		"help":
		"""I can help you understand neuroanatomy for medical practice!
Ask me about:
...
• Development: 'How does the corpus callosum develop?'""",
		"default":
		"I'm here to help you master neuroanatomy. Feel free to ask about any brain structure's anatomy, function, or clinical significance!"
	}
}
...
## @param structure_name: Medical name of the brain structure
	...
	match question_type:
		"function":
			structured_question = (
				"What are the main functions and clinical significance of the %s?"
				% current_structure
			)
		"location":
			structured_question = (
				"Describe the anatomical location of the %s using medical terminology."
				% current_structure
			)
		"connections":
			structured_question = (
				"What are the anatomical connections and pathways of the %s?" % current_structure
			)
		"clinical":
			structured_question = (
				"What are the clinical presentations when the %s is damaged?" % current_structure
			)
		"development":
			structured_question = (
				"Describe the embryological development of the %s." % current_structure
			)
		_:
...
## @returns: Array of conversation entries with medical Q&A
	...
	if not current_structure.is_empty():
		...
		if "function" in lower_question or "do" in lower_question or "role" in lower_question:
			...
			"location" in lower_question or "where" in lower_question or "anatomy" in lower_question
		):
			mock_response = structure_responses.get("location", "")
		elif (
			"connect" in lower_question
			or "pathway" in lower_question
			or "project" in lower_question
		):
			mock_response = structure_responses.get("connections", "")
		elif (
			"damage" in lower_question
			or "clinical" in lower_question
			or "lesion" in lower_question
			or "disease" in lower_question
		):
...
func _initialize_ai_service() -> void:
	...
	if OS.is_debug_build():
		print(
			(
				"[AI] Debug: Medical AI Assistant initialized with provider: %s"
...
func _on_api_response_received(
	http_result: int, _response_code: int, _headers: PackedStringArray, body: PackedByteArray
) -> void:
...
func _add_to_history(role: String, content: String) -> void:
	...
	conversation_history.append(
		{"role": role, "content": content, "timestamp": Time.get_unix_time_from_system()}
	)
