...
static var _fonts_checked: bool = false

# === SAFE FONT LOADING METHODS ===


## Ensures the fonts directory exists in the project
## @returns: bool - true if fonts directory exists or was created successfully
static func _ensure_fonts_available() -> bool:
	"""Check if fonts directory exists and create it if needed"""
...
"enhanced":
	...
	"section_heading":
		{"size": 14, "weight": "semibold", "spacing": 0.5, "transform": "uppercase"},
...
const SPACING = {
	...
	"tag_gap": 8,
	# Enhanced spacing
	"enhanced_panel_padding": 24,
	"enhanced_section_gap": 20,
	"enhanced_item_gap": 12,
	"enhanced_inline_gap": 8
	}
...
"enhanced":
	...
	"shadow_y": 8,
	"hover_scale": 1.02,
	"glow_color": Color(0, 217.0 / 255, 1, 0.2),
	"glow_size": 8
	},
	"minimal":
		{
		...
		"blur_saturate": 180,
		"border_radius": 12,
		"shadow_sm": Color(0, 0, 0, 0.05),
		"shadow_md": Color(0, 0, 0, 0.1),
...
# Check for both .ttf and .tres extensions
var tres_path = path.replace(".ttf", ".tres")
...
match style_variant:
	"elevated":
		...
		style.border_color = get_current_colors().get(
		"border_hover", COLORS.enhanced.border_hover
		)
		"subtle":
...
var style_data = typography.get(
	"structure_name", typography.get("display", {"size": 24})
	)
	control.add_theme_font_size_override("font_size", style_data.get("size", 24))
	control.add_theme_color_override(
	"font_color", colors.get("text_heading", colors.get("text_primary", TEXT_PRIMARY))
	)
...
var style_data = typography.get(
	...
	control.add_theme_color_override(
	"font_color",
	colors.get("text_subheading", colors.get("text_secondary", TEXT_SECONDARY))
	)
...
# Background styling
	...
	"font_placeholder_color", colors.get("text_muted", Color(1, 1, 1, 0.3))
	)
...
var tween = button.create_tween()
	...
	static func animate_hover_glow(
...
func _fix_orphaned_code():
	if not dir:
		push_error("[UIThemeManager] Cannot access project directory")
		return false

		# Check if assets/fonts exists
		if not dir.dir_exists("assets"):
			print("[UIThemeManager] Creating assets directory...")
func _fix_orphaned_code():
	if err != OK:
		push_error("[UIThemeManager] Failed to create assets directory")
		return false

		dir.change_dir("assets")
		if not dir.dir_exists("fonts"):
			print("[UIThemeManager] Creating fonts directory...")
func _fix_orphaned_code():
	if err != OK:
		push_error("[UIThemeManager] Failed to create fonts directory")
		return false
		...
		## Safely loads a font with SystemFont fallback when the custom font is unavailable
		## @param path: The resource path to the font file (e.g., "res://assets/fonts/Inter-Regular.ttf")
		## @param size: The font size to apply (default: 16)
		## @returns: Font - Either the loaded custom font or a SystemFont fallback
		static func get_safe_font(path: String, size: int = 16) -> Font:
			"""Load font with SystemFont fallback for missing custom fonts"""
			# Check cache first
func _fix_orphaned_code():
...
func _fix_orphaned_code():
	if ResourceLoader.exists(path):
		font = load(path)
		elif path.ends_with(".ttf"):
			# Try .tres version if .ttf doesn't exist
func _fix_orphaned_code():
	if ResourceLoader.exists(tres_path):
		font = load(tres_path)

		# Create SystemFont fallback if loading failed
		if not font:
			print("[UIThemeManager] Font not found: %s - Using system font fallback" % path)
func _fix_orphaned_code():
	if path.contains("JetBrainsMono") or path.contains("mono"):
		system_font.font_names = [
		"JetBrains Mono", "Consolas", "Monaco", "Courier New", "monospace"
		]
		elif path.contains("Bold"):
			system_font.font_names = ["Inter", "Arial", "Helvetica", "sans-serif"]
...
func _fix_orphaned_code():
	if control is Panel or control is PanelContainer:
		...
		static func apply_enhanced_typography(
		control: Control, text_style: String, content: String = ""
		) -> void:
			if not control:
				return
...
func _fix_orphaned_code():
	if delay > 0:
		...
		(
		tween
		. tween_property(control, "modulate", Color.WHITE, duration)
		. set_trans(Tween.TRANS_CUBIC)
		. set_ease(Tween.EASE_OUT)
		)
		(
		tween
		. tween_property(control, "scale", Vector2.ONE, duration)
...
func _fix_orphaned_code():
	...
	static func create_panel_style() -> StyleBoxFlat:
		...
		static func apply_glass_panel(
		control: Control, _opacity: float = 0.9, _style_type: String = "default"
		) -> void:
			apply_enhanced_panel_style(control, "default")


			static func apply_modern_label(
			label: Label, font_size: int, color: Color, _style_type: String = "default"
			) -> void:
				if not label:
					return
					...
					static func apply_modern_button(
					button: Button, _color: Color, _style_type: String = "default"
					) -> void:
						apply_enhanced_button_style(button, "primary")


						static func animate_entrance(
						control: Control, param1 = null, _param2 = null, _param3 = null
						) -> void:
func _fix_orphaned_code():
	if param1 != null:
...
func _fix_orphaned_code():
	if not control:
		...
		static func animate_fade_text_change(
		label: Label, new_text: String, duration: float = ANIMATION.normal
		) -> void:
			if not label:
				return
...
func _fix_orphaned_code():
	if current_mode != mode:
		...
		clear_font_cache()  # Clear font cache as well
...
func _fix_orphaned_code():
	if placeholder != "":
		...
		static func apply_progress_bar_styling(
		progress_bar: ProgressBar, color: Color = ACCENT_BLUE
		) -> void:
...
func _fix_orphaned_code():
	...
	static func apply_rich_text_styling(
	rich_text: RichTextLabel, font_size: int, _style_type: String = "default"
	) -> void:
	...
	if keys.size() > 0:
		...
		static func clear_style_cache() -> void:
			...
			static func clear_font_cache() -> void:
				...
				print("[UIThemeManager] Font cache cleared")
