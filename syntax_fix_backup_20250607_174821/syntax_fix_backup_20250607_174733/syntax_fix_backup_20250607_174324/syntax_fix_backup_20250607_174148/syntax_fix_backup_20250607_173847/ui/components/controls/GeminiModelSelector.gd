...
var ai_assistant: AIAssistantService
	...
	model_label.custom_minimum_size.x = 100
	...
	# Settings button
	settings_button = UIComponentFactory.create_button("⚙️", "icon")
	settings_button.tooltip_text = "Gemini AI Settings"
	settings_button.custom_minimum_size = Vector2(32, 32)
	add_child(settings_button)

	# Status indicator
	status_indicator = TextureRect.new()
	status_indicator.custom_minimum_size = Vector2(16, 16)
	status_indicator.expand_mode = TextureRect.EXPAND_KEEP_ASPECT_CENTERED
	status_indicator.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	...
	_set_status("unknown")
		...
		# Load current model
		current_model = gemini_service.get_model_name()
	else:
