...
var current_test = ""
	...
	info_test("AI API configuration should be validated separately")
		...
		if knowledge_service:
			...
			if (
				knowledge_service.has_method("get_structure")
				and knowledge_service.has_method("search_structures")
			):
				pass_test("Knowledge retrieval methods available")

				# Test a basic search
				if (
					knowledge_service.has_method("is_initialized")
					and knowledge_service.is_initialized()
				):
