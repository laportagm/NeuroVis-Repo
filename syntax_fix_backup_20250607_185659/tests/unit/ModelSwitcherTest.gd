...
# FIXME: Orphaned code - mock_node.name = "TestModel"
...
framework.assert_true(
model_switcher.is_model_visible("Test Model"), "Model should be visible by default"
)

# Toggle visibility
model_switcher.toggle_model_visibility("Test Model")
framework.assert_false(
model_switcher.is_model_visible("Test Model"), "Model should be hidden after toggle"
)
...
"Model count should remain unchanged"
)
...
# FIXME: Orphaned code - mock_node2.name = "TestModel2"
model_switcher.register_model(mock_node2, "Test Model 2")

# Hide all models
model_switcher.hide_all_models()
framework.assert_false(
model_switcher.is_model_visible("Test Model"), "First model should be hidden"
)
framework.assert_false(
model_switcher.is_model_visible("Test Model 2"), "Second model should be hidden"
)

# Show all models
model_switcher.show_all_models()
framework.assert_true(
model_switcher.is_model_visible("Test Model"), "First model should be visible"
)
framework.assert_true(
