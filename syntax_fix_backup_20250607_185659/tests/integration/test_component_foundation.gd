...
assert_not_equal(
FeatureFlags.is_enabled(FeatureFlags.UI_COMPONENT_POOLING),
original_state,
"Flag should toggle"
)
...
assert_false(
FeatureFlags.is_enabled(FeatureFlags.UI_STATE_PERSISTENCE), "Should disable feature"
)
...
# FIXME: Orphaned code - assert_true(typeof(all_flags) == TYPE_DICTIONARY, "Should return dictionary")
...
func test_feature_flags_basic() -> void:
	...
	# Test default flags
	assert_true(
	FeatureFlags.is_enabled(FeatureFlags.ADVANCED_ANIMATIONS),
	"Advanced animations should be enabled by default"
	)
	assert_false(
	FeatureFlags.is_enabled(FeatureFlags.UI_MODULAR_COMPONENTS),
	"Modular components should be disabled by default in production"
	)
...
func test_migration_compatibility() -> void:
	...
	assert_true(
	FeatureFlags.is_enabled(FeatureFlags.UI_LEGACY_PANELS), "Should enable legacy panels"
	)
	assert_false(
	FeatureFlags.is_enabled(FeatureFlags.UI_MODULAR_COMPONENTS),
	"Should disable modular components"
	)

	# Test new mode
	ComponentRegistry.set_legacy_mode(false)
	assert_false(
	FeatureFlags.is_enabled(FeatureFlags.UI_LEGACY_PANELS), "Should disable legacy panels"
	)
	assert_true(
	FeatureFlags.is_enabled(FeatureFlags.UI_MODULAR_COMPONENTS),
	...
	FeatureFlags.begin_migration(
	FeatureFlags.UI_LEGACY_PANELS, FeatureFlags.UI_MODULAR_COMPONENTS, true
	)
	assert_true(
	FeatureFlags.is_enabled(FeatureFlags.UI_MODULAR_COMPONENTS), "Should enable new system"
	)
	assert_true(
	FeatureFlags.is_enabled(FeatureFlags.UI_LEGACY_PANELS), "Should keep old system as fallback"
	)
