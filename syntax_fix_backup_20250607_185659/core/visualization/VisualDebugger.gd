...
# FIXME: Orphaned code - static var _enabled: bool = true

# Static settings - accessible from static methods
# FIXME: Orphaned code - static var text_color: Color = Color(1, 1, 0)  # Yellow
# FIXME: Orphaned code - static var ray_color: Color = Color(1, 0, 0)  # Red
# FIXME: Orphaned code - static var collision_color: Color = Color(0, 1, 0, 0.3)  # Green, semi-transparent
static var debug_material: StandardMaterial3D

# Static debug object collections
# FIXME: Orphaned code - static var debug_objects: Array = []
# FIXME: Orphaned code - static var mesh_highlights: Dictionary = {}
...
# FIXME: Orphaned code - main_scene._handle_selection = func(click_position):
...
func _fix_orphaned_code():
	...
	from: Vector3, to: Vector3, duration: float = 1.0, parent: Node3D = null
	) -> MeshInstance3D:
		_static_init()  # Ensure static variables are initialized
		if not Engine.is_editor_hint() and not _enabled:
			return null
...
func _fix_orphaned_code():
	for i in range(mesh_instance.mesh.get_surface_count()):
		...
		# Store in dictionary
		mesh_highlights[mesh_instance] = {"original_materials": original_materials, "timer": null}
