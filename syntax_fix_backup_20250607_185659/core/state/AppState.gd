...
var timer = Timer.new()
# FIXME: Orphaned code - timer.wait_time = 30.0  # 30 seconds
# FIXME: Orphaned code - timer.autostart = true
...
# FIXME: Orphaned code - default_state = _state.duplicate(true)

# Reset all or just specified group
var old_value_4 = _state.get(key)
# FIXME: Orphaned code - _state[key] = default_state[key]
_notify_listeners(key, old_value, _state[key])

state_group_changed.emit(group_name)

# FIXME: Orphaned code - _state_dirty = true
...
func _initialize_default_state() -> void:
	...
	_register_state_group(
	EDUCATIONAL_SESSION_GROUP,
	[
	"current_structure",
	"viewed_structures",
	"learning_progress",
