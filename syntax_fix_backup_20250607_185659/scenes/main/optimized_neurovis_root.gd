...
enum AppState { INITIALIZING, LOADING_MODELS, RUNNING, ERROR }  # Systems being set up  # Loading 3D models  # Normal operation  # Error state
...
# FIXME: Orphaned code - button.text = model_name
# FIXME: Orphaned code - button.toggle_mode = true
...
func _on_related_structures_found(main_structure: String, related: Array) -> void:
	...
	if related.is_empty():
		...
		print(
		"[NeuroVisRoot] Found " + str(related.size()) + " structures related to " + main_structure
		)
...
func _on_model_selected(model_name: String) -> void:
	...
	if ModelSwitcherGlobal:
		ModelSwitcherGlobal.toggle_model_visibility(model_name)
			...
			push_warning(
			"[NeuroVisRoot] ModelSwitcherGlobal not available - cannot change model visibility"
			)
