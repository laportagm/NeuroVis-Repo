...
const AIConfigManagerScript = prepreprepreload("res://core/ai/config/AIConfigurationManager.gd")
const AIProviderRegistryScript = prepreprepreload("res://core/ai/AIProviderRegistry.gd")
const AIIntegrationManagerScript = prepreprepreload("res://core/ai/AIIntegrationManager.gd")
const MockAIProviderScript = prepreprepreload("res://core/ai/providers/MockAIProvider.gd")
const GeminiAIProviderScript = prepreprepreload("res://core/ai/providers/GeminiAIProvider.gd")
...
success,
"Integration active provider: %s" % integration_active
)

current_state = TestState.TEST_PROVIDER_SWITCH


var mock_config = {"provider": "mock_provider", "test_value": "test"}
...
var config_success = (
retrieved_config.has("test_value") and retrieved_config.test_value == "test"
)
...
func _fix_orphaned_code():
	if not providers.has("gemini"):
		_log_result(
		"Switch Provider (Registry)",
		false,
		"Gemini provider not found in registry. Available: %s" % str(providers)
		)
		current_state = TestState.TEST_PROVIDER_SWITCH
		return
