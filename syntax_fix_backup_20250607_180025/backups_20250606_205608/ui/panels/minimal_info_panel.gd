...
const COLORS = {
	"background": Color(0.05, 0.05, 0.05, 0.95),  # Almost black background
	"border": Color(1, 1, 1, 0.15),  # Brighter border
...
# Typography scale
const FONT_SIZES = {"display": 28, "body": 15, "small": 14, "caption": 13, "micro": 12}

# Spacing system
const SPACING = {"xs": 4, "sm": 8, "md": 16, "lg": 24, "xl": 32}

# Animation settings
const ANIM = {"fast": 0.2, "normal": 0.3, "slow": 0.5, "curve": Tween.TRANS_CUBIC}

# Signals

var header_container: HBoxContainer
var structure_name: Label
var structure_category: Label
var bookmark_button: Button
var close_button: Button
var sections_container: VBoxContainer

# Section containers
var overview_section: VBoxContainer
...
var expanded_sections: Dictionary = {"functions": true, "related": false}

# Minimal color palette - VERY DIFFERENT FROM ENHANCED
	...
	var theme_indicator = PanelContainer.new()
	theme_indicator.custom_minimum_size = Vector2(0, 4)
	theme_indicator.position = Vector2(0, -10)
	theme_indicator.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	var indicator_style = StyleBoxFlat.new()
	indicator_style.bg_color = Color(1, 1, 1, 0.2)  # White bar
	theme_indicator.add_theme_stylebox_override("panel", indicator_style)
	sections_container.add_child(theme_indicator)
	...
	# Safety check for sections_container
	if not sections_container:
		push_error("[MINIMAL_PANEL] sections_container is null in _create_section")
		return section
...
func _create_header() -> void:
	...
	if not sections_container:
		push_error("[MINIMAL_PANEL] sections_container is null in _create_header")
		return

	header_container = HBoxContainer.new()
	sections_container.add_child(header_container)

	# Add visual theme indicator bar at top
...
func _create_section(
	title: String, collapsible: bool = false, expanded: bool = true, count: int = 0
) -> VBoxContainer:
...
func _update_content(data: Dictionary) -> void:
	...
	if structure_name and is_instance_valid(structure_name):
		structure_name.text = data.get("displayName", "Unknown")

	if structure_category and is_instance_valid(structure_category):
