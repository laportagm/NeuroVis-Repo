...
const FigmaDesign = preload("res://.claude/figma-exports/info-panel-design-spec.json")
...
# Signals
	...
	button.name = name.capitalize() + "Button"
	button.text = icon
	...
	tween.tween_property(self, "modulate", Color.WHITE, 0.4).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
...
func _update_panel_layout() -> void:
	"""Update panel size and position based on breakpoint"""
...
func _create_icon_button(name: String, icon: String) -> Button:
...
func _create_section(title: String, collapsible: bool = false, expanded: bool = true, premium: bool = false) -> VBoxContainer:
...
func _update_all_content(data: Dictionary) -> void:
