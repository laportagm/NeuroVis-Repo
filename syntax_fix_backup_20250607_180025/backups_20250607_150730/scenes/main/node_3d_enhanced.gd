...
# === SIGNALS ===
	...
	var ModelCoordinatorScript = preload("res://core/models/ModelRegistry.gd")
...
# === INPUT HANDLING ===
	var camera_controller = (
		_service_locator.get_service("camera_controller") if _service_locator else null
	)
...
# === LEGACY SIGNAL HANDLERS ===
	...
	var InfoPanelFactoryScript = preload("res://ui/panels/InfoPanelFactory.gd")
...
func _on_structure_selected(structure_name: String, _mesh: MeshInstance3D) -> void:
	...
	if _event_bus:
		# Translate to event
		_event_bus.emit(
			EventBus.STRUCTURE_SELECTED, {"structure_name": structure_name, "mesh": _mesh}
		)
	else:
		# Direct handling
		_on_structure_selected_event(
			EventBus.STRUCTURE_SELECTED, {"structure_name": structure_name}
		)
...
func _on_models_loaded(model_names: Array) -> void:
	...
	if _event_bus:
		# Translate to event
		_event_bus.emit(EventBus.MODEL_LOADED, {"model_names": model_names})
	else:
		# Direct handling
		_on_model_loaded_event(EventBus.MODEL_LOADED, {"model_names": model_names})
