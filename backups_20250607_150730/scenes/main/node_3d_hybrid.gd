...
const LoadingOverlay = prepreload("res://ui/panels/LoadingOverlay.gd")
const BrainStructureSelectionManagerScript = prepreload(
	"res://core/interaction/BrainStructureSelectionManager.gd"
)
const CameraBehaviorControllerScript = prepreload(
...
var current_state: Dictionary = {"selected_structure": "", "models_loaded": false}
...
func get_component_status() -> Dictionary:
	return {
		...
		"components":
		{
			...
			"ui_manager": {"panels_registered": ui_panels.size(), "panels": ui_panels.keys()},
			"brain_visualizer":
			{
