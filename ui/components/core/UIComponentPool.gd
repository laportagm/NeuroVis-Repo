...
var _stats: Dictionary = {
    "hits": 0,          # Pool hits (successful retrievals)
    "misses": 0,        # Pool misses (had to create new)
    "total_created": 0, # Total components created
    "total_reused": 0,  # Total components reused from pool
    "peak_active": 0    # Peak active components
}
...
func get_component(component_type: String, config: Dictionary = {}) -> Control:
    ...
    _active_components[instance_id] = component_type
...
func print_pool_stats() -> void:
    ...
    print("Pool Efficiency: " + str(snapped(float(stats["total_reused"]) / max(1, stats["total_created"] + stats["total_reused"]) * 100, 0.1)) + "%")
...
func _create_pool(component_type: String, size: int) -> void:
    ...
    print("[UIComponentPool] Creating pool for '" + component_type + "' with " + str(size) + " components")
