...
static func _create_control_item(item_config: Dictionary) -> Control:
	...
	match item_type:
		"button":
			return create_button(item_config.get("text", "Button"), item_config.get("style", "primary"), item_data)
		"slider":
			var container = HBoxContainer.new()
			var label = create_label(item_config.get("label", "Value:"))
			var slider = create_slider(item_data.get("min", 0), item_data.get("max", 1), item_data.get("value", 0.5))
			container.add_child(label)
			container.add_child(slider)
			return container
		"checkbox":
			return create_checkbox(item_config.get("text", "Option"), item_config.get("checked", false))
		_:
...
static func _create_form_field(field_config: Dictionary) -> Control:
	...
	match field_type:
		...
		"checkbox":
			field_input = create_checkbox(field_config.get("text", ""), field_config.get("checked", false))
		"slider":
			field_input = create_slider(field_config.get("min", 0), field_config.get("max", 1), field_config.get("value", 0.5))
		_:
