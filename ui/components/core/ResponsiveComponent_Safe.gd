...
func _adapt_to_viewport() -> void:
	...
	if old_breakpoint != current_breakpoint:
		breakpoint_changed.emit(BREAKPOINT_NAMES[old_breakpoint], BREAKPOINT_NAMES[current_breakpoint])
		_log("Breakpoint changed: " + BREAKPOINT_NAMES[old_breakpoint] + " -> " + BREAKPOINT_NAMES[current_breakpoint])
