...
func _setup_layout() -> void:
	...
	main_container.add_theme_constant_override("separation", UIThemeManager.get_spacing("enhanced_section_gap"))
...
func _create_ui_fragments() -> void:
	...
	content_fragment = ComponentRegistry.create_component("content", {
	...
	actions_fragment = ComponentRegistry.create_component("actions", {
		"buttons": [
...
func _on_header_action(action: String, data: Dictionary = {}) -> void:
	...
	match action:
		...
		"bookmark":
			...
			if header_fragment and header_fragment.has_method("set_bookmark_state"):
				header_fragment.set_bookmark_state(is_bookmarked)
			action_triggered.emit("bookmark", {"structure_id": structure_data.get("id", ""), "bookmarked": is_bookmarked})
		"share":
...
func _on_mouse_entered() -> void:
	...
	if FeatureFlags.is_enabled(FeatureFlags.UI_ADVANCED_INTERACTIONS):
		...
		var hover_tween = StyleEngine.create_scale_animation(
			...
			Vector2.ONE, 
			Vector2.ONE * 1.02, 
			StyleEngine.get_animation_duration("micro")
		)
		hover_tween.play()
