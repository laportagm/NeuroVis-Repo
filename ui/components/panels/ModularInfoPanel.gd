...
func _create_title_bar() -> void:
	...
	# Close button
	if closeable:
		close_button = UIComponentFactory.create_button("✕", "icon", {
...
func _create_section_header(section_id: String, config: Dictionary) -> Control:
	...
	# Section title
	var title = UIComponentFactory.create_label(config.get("title", section_id.capitalize()), "subheading")
	...
	if collapsible_sections and config.get("collapsible", true):
		var collapse_btn = UIComponentFactory.create_button("▼", "icon", {
...
func _update_rich_text_content(rich_text: RichTextLabel, data: Dictionary) -> void:
	...
	if subtitle != "":
		text = "[font_size=18][b]" + text + "[/b][/font_size]\n[font_size=12][color=#B0B0C0]" + subtitle + "[/color][/font_size]"
...
func _update_buttons_content(buttons_container: VBoxContainer, data: Dictionary) -> void:
	...
	for button_data in buttons:
		var button = UIComponentFactory.create_button(
			button_data.get("text", "Button"),
			button_data.get("style", "secondary")
		)
