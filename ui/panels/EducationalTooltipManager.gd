...
var educational_tooltips: Dictionary = {
	"brain_structures": {
		"Hippocampus": {
			"title": "🧠 Hippocampus",
			"description": "Critical for learning and memory formation. Part of the limbic system that converts short-term memories to long-term storage.",
		...
		"Thalamus": {
			"title": "⚡ Thalamus", 
			"description": "The brain's relay station. Processes and forwards sensory information to the cerebral cortex.",
		...
		"Striatum": {
			"title": "🎯 Striatum",
			"description": "Part of the basal ganglia controlling movement, habit formation, and reward processing.",
	...
	"ui_elements": {
		"search_field": {
			"title": "🔍 Smart Search",
			"description": "Search brain structures, functions, or anatomical terms. Use keywords like 'memory', 'movement', or specific structure names.",
			"category": "Navigation",
			"difficulty": "beginner",
			"quick_facts": [
		...
		"difficulty_indicator": {
			...
			"description": "Content complexity indicator to help pace your learning journey through neuroanatomy.",
		...
		"bookmark_system": {
			...
			"description": "Save important structures for quick review and create your personal study collection.",
...
func _setup_tooltip_ui() -> void:
	...
	UIThemeManager.apply_modern_label(tooltip_header, UIThemeManager.FONT_SIZE_MEDIUM, UIThemeManager.TEXT_PRIMARY, "heading")
	...
	UIThemeManager.apply_rich_text_styling(tooltip_body, UIThemeManager.FONT_SIZE_SMALL, "description")
	...
	UIThemeManager.apply_modern_label(tooltip_difficulty, UIThemeManager.FONT_SIZE_TINY, UIThemeManager.TEXT_SECONDARY, "caption")
...
func show_tooltip(target: Control, tooltip_id: String, category: String = "ui_elements") -> void:
	...
	if tooltip_data.is_empty():
		print("[TOOLTIP_MANAGER] No tooltip data found for: %s in category: %s" % [tooltip_id, category])
		return
...
func _update_tooltip_content() -> void:
	...
	var content = "[font_size=%d][color=%s]%s[/color][/font_size]" % [
	...
	if learning_tip != "":
		content += "\n\n[font_size=%d][color=%s][b]💡 Learning Tip:[/b][/color][/font_size]" % [
			UIThemeManager.FONT_SIZE_TINY,
			UIThemeManager.ACCENT_YELLOW.to_html()
		]
...
func _show_with_animation() -> void:
	...
	tween.tween_property(self, "modulate:a", 1.0, UIThemeManager.ANIM_DURATION_FAST).set_ease(Tween.EASE_OUT)
...
func hide_tooltip() -> void:
	...
	var tween = create_tween()
	tween.set_parallel(true)
	tween.tween_property(self, "modulate:a", 0.0, UIThemeManager.ANIM_DURATION_FAST).set_ease(Tween.EASE_IN)
	tween.tween_property(self, "scale", Vector2(0.8, 0.8), UIThemeManager.ANIM_DURATION_FAST).set_ease(Tween.EASE_IN)
