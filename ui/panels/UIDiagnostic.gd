	...
	if child_count == 0:
		...
		for i in range(child_count):
			...
			if child == null:
				...
				print(" - " + child.name + " (" + child.get_class() + "), visible: " + str(child.visible))
...
func check_structure_selection_flow() -> void:
	...
	if main_scene.knowledge_base == null:
		printerr("UIDiagnostic Error: Knowledge base reference is null in check_structure_selection_flow()")
		return
	...
	if main_scene.neural_net == null:
		printerr("UIDiagnostic Error: Neural net reference is null in check_structure_selection_flow()")
		return
	...
	if info_panel == null:
		printerr("UIDiagnostic Error: Info panel is null in check_structure_selection_flow(), can't check signal connections")
	else:
			...
			if signal_info.name == "panel_closed":
				...
				for connection in connections:
					...
					if target_object == null:
						...
						print(" - Connected to: " + target_object.name + " -> " + str(connection.callable))
...
func test_info_panel_with_data() -> void:
	...
	var test_data = {
		"id": "test_structure",
		"displayName": "Test Structure",
		"shortDescription": "This is a comprehensive test structure with a longer description to test text wrapping and display capabilities of the info panel.",
		"functions": [
	...
	if not info_panel.has_method("display_structure_data"):
		printerr("UIDiagnostic Error: Info panel doesn't have display_structure_data method in test_info_panel_with_data()")
		return
	
	_safe_call_method(info_panel, "display_structure_data", [test_data], "test_info_panel_with_data")
...
func trace_info_panel_calls() -> void:
	...
	if not info_panel.has_method("display_structure_data"):
		printerr("UIDiagnostic Error: Info panel doesn't have display_structure_data method in trace_info_panel_calls()")
		return
...
func _safe_call_method(object: Object, method_name: String, args: Array, context: String) -> void:
	if not object.has_method(method_name):
		printerr("UIDiagnostic Error: Object doesn't have method '" + method_name + "' in " + context + "()")
		return
