...
var learning_achievements: Dictionary = {
	"first_structure_viewed": {
		...
		"message": "You've viewed your first brain structure! Keep exploring to learn more about neuroanatomy.",
		"icon": "🏆",
...
var learning_tips: Array = [
	{
		"title": "💡 Study Tip",
		"message": "Try studying structures by function rather than just memorizing names. Ask yourself: 'What does this structure DO?'",
		"category": "study_strategy"
	},
	{
		"title": "💡 Memory Tip",
		"message": "Create mental stories connecting brain regions. For example: 'The hippocampus is like a librarian, filing memories for the cortex to access.'",
		"category": "memorization"
	},
	{
		"title": "💡 Learning Tip",
		"message": "Use the difficulty indicators! Start with green (beginner) structures and work your way up to red (advanced) content.",
		"category": "progression"
	},
	{
		"title": "💡 Review Tip", 
		"message": "Bookmark challenging structures and review them regularly. Spaced repetition helps build long-term memory!",
		"category": "review"
	},
	{
		"title": "💡 Understanding Tip",
		"message": "Connect anatomy to real life! When you learn about the motor cortex, think about typing, walking, or playing sports.",
		"category": "connection"
	}
]
...
func show_progress_update(milestone_type: String, current_value: int, target_value: int = 0) -> void:
	...
	match milestone_type:
		...
		"study_time":
			message = "You've spent %d minutes learning today. Excellent dedication!" % current_value
		"bookmarks_created":
			message = "You've bookmarked %d structures for review. Building a great study collection!" % current_value
		_:
...
func _build_notification_ui(data: Dictionary) -> Control:
	...
	UIThemeManager.apply_modern_label(title_label, UIThemeManager.FONT_SIZE_MEDIUM, UIThemeManager.TEXT_PRIMARY, "heading")
	...
	UIThemeManager.apply_modern_label(message_label, UIThemeManager.FONT_SIZE_SMALL, UIThemeManager.TEXT_SECONDARY)
	...
	if data["type"] in [NotificationType.ACHIEVEMENT, NotificationType.DISCOVERY]:
		...
		notification.mouse_entered.connect(func(): UIThemeManager.animate_hover_glow(notification, border_color, 0.2))
		notification.mouse_exited.connect(func(): UIThemeManager.animate_hover_glow_off(notification))
...
func _animate_notification_entrance(notification: Control) -> void:
	...
	tween.tween_property(notification, "position:x", 0, SLIDE_IN_DURATION).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_BACK)
...
func _animate_notification_exit(notification: Control) -> void:
	...
	tween.tween_property(notification, "modulate:a", 0.0, SLIDE_OUT_DURATION).set_ease(Tween.EASE_IN)
