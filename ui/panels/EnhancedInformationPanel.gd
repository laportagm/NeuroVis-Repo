...
var section_states: Dictionary = {
	"description": true,    # Always expanded
	"functions": true,      # Default expanded  
	"connections": false,   # Default collapsed
	"clinical": false       # Default collapsed
}
...
func _setup_panel_structure() -> void:
	...
	scroll_container.add_theme_constant_override("margin_left", UIThemeManager.get_spacing("enhanced_panel_padding"))
	scroll_container.add_theme_constant_override("margin_right", UIThemeManager.get_spacing("enhanced_panel_padding"))
	scroll_container.add_theme_constant_override("margin_top", UIThemeManager.get_spacing("enhanced_panel_padding"))
	scroll_container.add_theme_constant_override("margin_bottom", UIThemeManager.get_spacing("enhanced_panel_padding"))
	add_child(scroll_container)
	
	# Main content container
	content_container = VBoxContainer.new()
	content_container.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	content_container.add_theme_constant_override("separation", UIThemeManager.get_spacing("enhanced_section_gap"))
...
func _create_header_section() -> void:
	...
	action_buttons_container.add_theme_constant_override("separation", UIThemeManager.get_spacing("enhanced_inline_gap"))
...
func _create_functions_section() -> void:
	...
	functions_list.add_theme_constant_override("separation", UIThemeManager.get_spacing("enhanced_inline_gap"))
...
func _create_connections_section() -> void:
	...
	connections_list.add_theme_constant_override("separation", UIThemeManager.get_spacing("enhanced_inline_gap"))
...
func _setup_accessibility() -> void:
	...
	close_button.add_theme_color_override("font_focus_color", UIThemeManager.get_color("text_heading"))
	bookmark_button.add_theme_color_override("font_focus_color", UIThemeManager.get_color("text_heading"))
	share_button.add_theme_color_override("font_focus_color", UIThemeManager.get_color("text_heading"))
...
func display_structure_info(structure_data: Dictionary) -> void:
	...
	var structure_name = structure_data.get("displayName", structure_data.get("name", "Unknown Structure"))
	structure_name_label.text = structure_name
	
	# Update description
	var description = structure_data.get("shortDescription", structure_data.get("description", "No description available."))
...
func _update_functions_list(functions: Array) -> void:
	...
	if functions.size() > 0:
		...
		functions_header.text = ("▼" if section_states.functions else "▶") + " FUNCTIONS (" + str(functions.size()) + ")"
...
func _update_connections_list(connections: Array) -> void:
	...
	if connections.size() > 0:
		connections_header.text = ("▼" if section_states.connections else "▶") + " CONNECTIONS (" + str(connections.size()) + ")"
...
func _toggle_section(section_name: String) -> void:
	...
	if new_state:
		...
		tween.tween_property(content_node, "modulate", Color.WHITE, UIThemeManager.get_animation_duration("content_fade_duration"))
	else:
		var tween = create_tween()
		tween.tween_property(content_node, "modulate", Color.TRANSPARENT, UIThemeManager.get_animation_duration("content_fade_duration"))
