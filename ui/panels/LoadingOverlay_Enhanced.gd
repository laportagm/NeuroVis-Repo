...
var loading_tips: Array = [
	{
		"title": "🧠 Brain Basics",
		"content": "The human brain contains approximately 86 billion neurons, each connected to thousands of others, creating a network more complex than any computer ever built.",
		"category": "anatomy"
	},
	{
		"title": "🔬 Neuroplasticity",
		"content": "Your brain can reorganize itself throughout your life! This amazing ability, called neuroplasticity, means learning actually changes your brain's structure.",
		"category": "science"
	},
	{
		"title": "💭 Memory Formation",
		"content": "The hippocampus acts like a librarian, temporarily storing new memories before transferring important ones to the cortex for long-term storage.",
		"category": "function"
	},
	{
		"title": "⚡ Neural Speed",
		"content": "Nerve impulses can travel at speeds up to 120 meters per second! That's faster than most cars drive in the city.",
		"category": "physiology"
	},
	{
		"title": "🎯 Learning Strategy",
		"content": "When studying neuroanatomy, try connecting structures to their functions. Ask yourself: 'What does this part DO?' rather than just memorizing names.",
		"category": "study_tips"
	},
	{
		"title": "🌟 Did You Know?",
		"content": "The brain uses about 20% of your body's total energy, despite being only 2% of your body weight. It's truly a power-hungry supercomputer!",
		"category": "facts"
	},
	{
		"title": "🔄 Brain Waves",
		"content": "Your brain produces electrical waves that can be measured. Different wave patterns correspond to different states like focus, relaxation, or deep sleep.",
		"category": "physiology"
	},
	{
		"title": "🧩 Hemispheres", 
		"content": "While both brain hemispheres work together, they do have some specialized functions. The left is often associated with language, the right with spatial processing.",
		"category": "anatomy"
	}
]
...
var loading_phases: Dictionary = {
	...
	"loading_models": {
		...
		"progress": 40,
		"tasks": ["Loading brain anatomy meshes", "Processing model data", "Preparing visualization"]
	},
	"loading_data": {
		"name": "Loading Educational Content",
		"progress": 70,
		"tasks": ["Loading anatomical database", "Preparing learning materials", "Setting up knowledge base"]
	},
	"finalizing": {
...
func _apply_enhanced_styling() -> void:
	...
	UIThemeManager.apply_modern_label(app_title, UIThemeManager.FONT_SIZE_HERO, UIThemeManager.TEXT_PRIMARY, "title")
	UIThemeManager.apply_modern_label(app_subtitle, UIThemeManager.FONT_SIZE_MEDIUM, UIThemeManager.TEXT_SECONDARY, "caption")
	
	# Progress styling
	UIThemeManager.apply_modern_label(current_task_label, UIThemeManager.FONT_SIZE_LARGE, UIThemeManager.ACCENT_BLUE, "heading")
	UIThemeManager.apply_modern_label(progress_label, UIThemeManager.FONT_SIZE_SMALL, UIThemeManager.TEXT_SECONDARY, "caption")
	UIThemeManager.apply_modern_label(progress_percentage, UIThemeManager.FONT_SIZE_MEDIUM, UIThemeManager.ACCENT_CYAN)
	UIThemeManager.apply_progress_bar_styling(progress_bar, UIThemeManager.ACCENT_BLUE)
	
	# Educational content styling
	UIThemeManager.apply_modern_label(tip_header, UIThemeManager.FONT_SIZE_H3, UIThemeManager.ACCENT_YELLOW, "subheading")
	UIThemeManager.apply_rich_text_styling(tip_content, UIThemeManager.FONT_SIZE_MEDIUM, "description")
	...
	UIThemeManager.apply_modern_label(tip_indicator, UIThemeManager.FONT_SIZE_SMALL, UIThemeManager.TEXT_SECONDARY, "caption")
...
func _animate_progress_update() -> void:
	...
	# Animate progress bar
	loading_tween.tween_property(progress_bar, "value", target_progress, 0.5).set_ease(Tween.EASE_OUT)
	
	# Animate percentage text
	loading_tween.tween_method(_update_percentage_text, current_progress, target_progress, 0.5).set_ease(Tween.EASE_OUT)
...
func _animate_entrance() -> void:
	...
	tween.tween_property(self, "modulate:a", 1.0, UIThemeManager.ANIM_DURATION_STANDARD).set_ease(Tween.EASE_OUT)
	
	# Stagger content animations
	if logo_container:
		UIThemeManager.animate_entrance(logo_container, 0.2, UIThemeManager.ANIM_DURATION_STANDARD, "fade_scale")
	
	if progress_container:
		UIThemeManager.animate_entrance(progress_container, 0.4, UIThemeManager.ANIM_DURATION_STANDARD, "slide_up")
...
func _animate_exit() -> void:
	...
	# Fade out everything
	tween.tween_property(self, "modulate:a", 0.0, UIThemeManager.ANIM_DURATION_STANDARD).set_ease(Tween.EASE_IN)
	
	# Scale down content
	if content_container:
		tween.tween_property(content_container, "scale", Vector2(0.9, 0.9), UIThemeManager.ANIM_DURATION_STANDARD).set_ease(Tween.EASE_IN)
...
func _update_educational_content() -> void:
	...
	# Update tip content
	if tip_content:
		var formatted_content = "[center][font_size=%d][color=%s][b]%s[/b][/color][/font_size][/center]\n\n[font_size=%d][color=%s]%s[/color][/font_size]" % [
