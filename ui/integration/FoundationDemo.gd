...
func _demo_create_info_panel() -> void:
	...
	if panel:
		...
		_log_to_results("   Creation time: %s" % Time.get_datetime_string_from_unix_time(panel.get_meta("creation_time", 0)))
...
func _demo_state_persistence() -> void:
	...
	if not restored_state.is_empty():
		...
		_log_to_results("   Expanded sections: %s" % str(restored_state.get("expanded_sections", [])))
...
func _demo_legacy_comparison() -> void:
	...
	_log_to_results("   Legacy panels: %s" % ("✅" if FeatureFlags.is_enabled(FeatureFlags.UI_LEGACY_PANELS) else "❌"))
	_log_to_results("   Modular components: %s" % ("✅" if FeatureFlags.is_enabled(FeatureFlags.UI_MODULAR_COMPONENTS) else "❌"))
	
	# Create component in legacy mode
	var legacy_panel = ComponentRegistry.create_component("info_panel", {})
	_log_to_results("📋 Legacy panel created: %s" % (legacy_panel.get_class() if legacy_panel else "Failed"))
