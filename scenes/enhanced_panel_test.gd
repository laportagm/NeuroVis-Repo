...
var test_structure_data = {
	...
	"shortDescription": "The hippocampus is a critical brain region involved in learning, memory formation, and spatial navigation. It's part of the limbic system and plays a crucial role in converting short-term memories into long-term memories.",
	"functions": [
	...
	"clinicalNotes": "Damage to the hippocampus can result in severe memory impairments, particularly affecting the ability to form new episodic memories (anterograde amnesia). This region is also one of the first areas affected in Alzheimer's disease."
}
...
func _on_bookmark_toggled(structure_id: String, bookmarked: bool):
	print("[Test] Structure '%s' %s" % [structure_id, "bookmarked" if bookmarked else "unbookmarked"])