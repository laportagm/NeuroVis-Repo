...
func _get_autoload_references() -> void:
    ...
    if ResourceLoader.exists(UIComponentFactoryPath):
        ...
        push_warning("[DebugManager] Warning: UIComponentFactory not found at " + UIComponentFactoryPath)
...
func _register_debug_commands() -> void:
    ...
    if not DebugCmd:
        push_warning("[DebugManager] Warning: DebugCmd not available, skipping command registration")
        return
    ...
    DebugCmd.register_command("ai_provider", _debug_ai_provider, "List or change AI providers [provider_id]")
    
    # Foundation testing commands
    DebugCmd.register_command("test_foundation", _debug_test_foundation, "Test foundation layer components")
    DebugCmd.register_command("test_components", _debug_test_components, "Test UI component system")
    DebugCmd.register_command("test_phase3", _debug_test_phase3, "Test Phase 3 features (StyleEngine & AdvancedInteractions)")
...
func _debug_ai_status(args: Array = []) -> void:
    ...
    print("[DebugManager] Info: Available Providers: %s" % str(ai_integration.get_available_providers()))
...
func _debug_ai_setup(args: Array = []) -> void:
    ...
    print("[DebugManager] Info: Showing setup dialog for provider: %s" % (provider_id if not provider_id.is_empty() else "default"))
...
func _debug_ai_provider(args: Array = []) -> void:
    ...
    if args.size() == 0:
        ...
        for provider in providers:
            ...
            print("[DebugManager] Info: %s %s - %s" % ["►" if provider == active else " ", provider, str(status)])
        print("[DebugManager] Info: =============================\n")
...
func _debug_test_foundation(args: Array = []) -> void:
    ...
    if FeatureFlags and FeatureFlags.has_method("is_enabled"):
        ...
        if FeatureFlags.has_method("get") or "UI_MODULAR_COMPONENTS" in FeatureFlags:
            print("[DebugManager] Info:   - UI_MODULAR_COMPONENTS: %s" % ("Enabled" if FeatureFlags.is_enabled(FeatureFlags.UI_MODULAR_COMPONENTS) else "Disabled"))
        if FeatureFlags.has_method("is_enabled"):
            print("[DebugManager] Info:   - MULTI_STRUCTURE_SELECTION: %s" % ("Enabled" if FeatureFlags.is_enabled("multi_structure_selection") else "Disabled"))
        if FeatureFlags.has_method("get") or "UI_STYLE_ENGINE" in FeatureFlags:
            print("[DebugManager] Info:   - UI_STYLE_ENGINE: %s" % ("Enabled" if FeatureFlags.is_enabled(FeatureFlags.UI_STYLE_ENGINE) else "Disabled"))
    else:
        push_warning("[DebugManager] Warning: ✗ FeatureFlags: Not available")
