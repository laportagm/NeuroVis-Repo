...
var AIIntegrationManager = load("res://core/ai/AIIntegrationManager.gd")
...
func _register_foundation_debug_commands() -> void:
    ...
    debug_cmd.register_command("test_foundation", self, "_debug_test_foundation", "Test foundation layer")
    debug_cmd.register_command("test_components", self, "_debug_test_components", "Test UI component system")
    ...
    debug_cmd.register_command("ai_provider", self, "_debug_ai_provider", "List or change AI providers")
...
func _debug_test_foundation(args: Array = []) -> void:
    ...
    if FeatureFlags and FeatureFlags.has_method("is_enabled"):
        ...
        print("  - MULTI_STRUCTURE_SELECTION: %s" % ("Enabled" if FeatureFlags.is_enabled(FeatureFlags.MULTI_STRUCTURE_SELECTION) else "Disabled"))
